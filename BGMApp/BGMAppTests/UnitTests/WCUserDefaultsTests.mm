...
//
//  WCUserDefaultsTests.mm
//  BGMAppUnitTests
//
//  Copyright © 2026 Wavecraft contributors
//
//  Covers WCUserDefaults's hotkey-binding storage (BGMHotkeyAction/BGMHotkeyBinding round trips,
//  defaults, clamping) and the BGMHotkeyBinding helper functions. This is plain Foundation/plist
//  logic with no CoreAudio HAL dependency, unlike WCHotkeys itself (which can't be unit tested --
//  see WCTapRouteTests.mm's own comment on why -- because it touches real system state:
//  AXIsProcessTrusted(), NSWorkspace.frontmostApplication, and a real global NSEvent monitor).
//  WCUserDefaults supports this via initWithDefaults:nil, which keeps everything in memory
...
@interface WCUserDefaultsTests : XCTestCase
@end

@implementation WCUserDefaultsTests {
    WCUserDefaults* defaults;
}

- (void) setUp {
    [super setUp];
    defaults = [[WCUserDefaults alloc] initWithDefaults:nil];
}
