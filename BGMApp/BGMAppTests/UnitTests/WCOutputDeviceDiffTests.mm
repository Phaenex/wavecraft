...
//
//  WCOutputDeviceDiffTests.mm
//  BGMAppUnitTests
//
//  Copyright © 2026 Wavecraft contributors
//
//  Unlike WCTapRouteTests.mm (deliberately limited to construction/validation only -- see its own
//  comment for why the real tap/WCPlayThrough pipeline can't be exercised in this mocked target),
...
@interface WCOutputDeviceDiffTests : XCTestCase
@end

@implementation WCOutputDeviceDiffTests
...
- (void) assertDiffWithCurrent:(std::vector<AudioObjectID>)current
    ...
    WCOutputDeviceDiff diff = BGMComputeOutputDeviceDiff(current, desired);
...
// only B and C. A must never appear in either list -- WCAppOutputRoutingController relies on that
