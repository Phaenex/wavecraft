...
@implementation WCXPCHelperService {
...
+ (void) withBGMAppRemoteProxy:(void (^)(id))block errorHandler:(void (^)(NSError* error))errorHandler {
    // Retry by default
    return [WCXPCHelperService withBGMAppRemoteProxy:block errorHandler:errorHandler retryOnError:YES];
}
...
+ (void) withBGMAppRemoteProxy:(void (^)(id))block errorHandler:(void (^)(NSError* error))errorHandler retryOnError:(BOOL)retry {
    ...
    if (!sBGMAppConnection && sBGMAppEndpoint) {
        ...
        @synchronized(self) {
            ...
            [sBGMAppConnection setRemoteObjectInterface:[NSXPCInterface interfaceWithProtocol:@protocol(WCAppXPCProtocol)]];
    ...
    if (sBGMAppConnection) {
        id proxy = [sBGMAppConnection remoteObjectProxyWithErrorHandler:^(NSError* error) {
            if (retry) {
                DebugMsg("WCXPCHelperService::withBGMAppRemoteProxy: %s error=<%lu, %s>",
                ...
                // Retry the message.
                [WCXPCHelperService withBGMAppRemoteProxy:block errorHandler:errorHandler retryOnError:NO];
            } else {
                NSLog(@"WCXPCHelperService::withBGMAppRemoteProxy: Error sending message to BGMApp: %@", error);
                errorHandler(error);
            }
        }];
        ...
        errorHandler([WCXPCHelperService errorWithCode:kBGMXPC_MessageFailure description:@"No connection to BGMApp"]);
    }
}
...
- (void) unsetBGMDeviceAsDefault {
    ...
    if (sBGMAppConnection) {
        DebugMsg("WCXPCHelperService::unsetBGMDeviceAsDefault: BGMApp connected. Doing nothing.");
        return;
    }
    ...
    if (outputDevice == kAudioObjectUnknown) {
        ...
        DebugMsg("WCXPCHelperService::unsetBGMDeviceAsDefault: No device to set. Doing nothing.");
        return;
    }

    // If BGMDevice has been left as the default device, change it to the real output device.
    BGMLogAndSwallowExceptions("WCXPCHelperService::unsetBGMDeviceAsDefault", ([&] {
        NSLog(@"WCXPCHelperService::unsetBGMDeviceAsDefault: Changing default device to %u",
              outputDevice);

        WCBackgroundMusicDevice().UnsetAsOSDefault(outputDevice);
    }));
}
...
- (void) registerAsBGMAppWithListenerEndpoint:(NSXPCListenerEndpoint*)endpoint reply:(void (^)(void))reply {
    ...
    DebugMsg("WCXPCHelperService::registerAsBGMAppWithListenerEndpoint: Received BGMApp listener endpoint");
...
- (void) unregisterAsBGMApp {
    ...
    DebugMsg("WCXPCHelperService::unregisterAsBGMApp: Destroying connection to BGMApp");
...
- (void) startBGMAppPlayThroughSyncWithReply:(void (^)(NSError*))reply forUISoundsDevice:(BOOL)isUI {
    ...
    __block NSError* replyToBGMDriver = [WCXPCHelperService errorWithCode:kBGMXPC_InternalError
    ...
    DebugMsg("WCXPCHelperService::startBGMAppPlayThroughSyncWithReply: Waiting for BGMApp to start IO on the output device");
    
    // Send the message to BGMApp
    [WCXPCHelperService withBGMAppRemoteProxy:^(id remoteObjectProxy) {
        ...
        replyToBGMDriver = [WCXPCHelperService errorWithCode:kBGMXPC_MessageFailure
    ...
    if (err != 0) {
        replyToBGMDriver = [WCXPCHelperService errorWithCode:kBGMXPC_Timeout
                                                  description:@"Timed out waiting for BGMApp"];
    }

    // Return the reply to BGMDriver
    DebugMsg("WCXPCHelperService::startBGMAppPlayThroughSyncWithReply: Reply to BGMDriver: %s",
...
- (void) setOutputDeviceToMakeDefaultOnAbnormalTermination:(AudioObjectID)deviceID {
    ...
    DebugMsg("WCXPCHelperService::setOutputDeviceToMakeDefaultOnAbnormalTermination: ID set to %u",
             deviceID);
}
...
#if DEBUG
    if ([connection effectiveUserIdentifier] != [WCXPCListenerDelegate _coreaudiodUID]) {
        DebugMsg("WCXPCHelperService::debugWarnIfCalledByBGMDriver: A method intended for BGMDriver only was (probably) called "
...
#if DEBUG
    if ([connection effectiveUserIdentifier] == [WCXPCListenerDelegate _coreaudiodUID]) {
        DebugMsg("WCXPCHelperService::debugWarnIfCalledByBGMDriver: A method intended for BGMApp only was (probably) called by BGMDriver");
        NSLog(@"%@", [NSThread callStackSymbols]);
    }
#endif
}
