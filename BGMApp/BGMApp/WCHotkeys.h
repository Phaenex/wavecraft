...
// WCUserDefaults.h imports this header (to declare hotkeyBindingForAction:/setHotkeyBinding:
// forAction:, which need BGMHotkeyBinding/BGMHotkeyAction's real definitions, not just a pointer
// type), so importing it back here would be circular. This header only ever uses WCUserDefaults
// as a pointer type, so a forward declaration is enough -- WCHotkeys.mm imports the real header.
