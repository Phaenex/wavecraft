...
//
//  BGMOutputDeviceDiff.h
//  BGMApp
//
//  Copyright © 2026 Wavecraft contributors
//
//  Pure decision logic for BGMAppOutputRoutingController's multi-device reconciliation, pulled out
//  of applyRouteForBundleID:deviceUIDs:appName: specifically so it can be unit tested without a
//  real BGMTapRoute/CoreAudio tap -- unlike almost everything else this class touches, computing
//  which devices to add/remove has no HAL dependency at all, so there's no reason for it to share
//  BGMTapRouteTests.mm's "construction/validation only" limitation.
//
 ...
 */
BGMOutputDeviceDiff BGMComputeOutputDeviceDiff(const std::vector<AudioObjectID>& inCurrentDevices,
