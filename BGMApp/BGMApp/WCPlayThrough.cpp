...
WCPlayThrough::WCPlayThrough(BGMAudioDevice inInputDevice, BGMAudioDevice inOutputDevice)
:
...
WCPlayThrough::~WCPlayThrough()
{
    CAMutex::Locker stateLocker(mStateMutex);

    BGMLogAndSwallowExceptionsMsg("WCPlayThrough::~WCPlayThrough", "Deactivate", [&]() {
        Deactivate();
    });
    ...
    //       to the WCPlayThrough object instead of raw pointers.
    DeallocateBuffer();
    ...
    if(mOutputDeviceIOProcSemaphore != SEMAPHORE_NULL)
    {
        ...
        BGM_Utils::LogIfMachError("WCPlayThrough::~WCPlayThrough", "semaphore_destroy", theError);
    }
}

void    WCPlayThrough::Init(BGMAudioDevice inInputDevice, BGMAudioDevice inOutputDevice)
{
    BGMAssert(mInputDeviceIOProcState.is_lock_free(),
              "WCPlayThrough::WCPlayThrough: !mInputDeviceIOProcState.is_lock_free()");
    BGMAssert(mOutputDeviceIOProcState.is_lock_free(),
              "WCPlayThrough::WCPlayThrough: !mOutputDeviceIOProcState.is_lock_free()");
    BGMAssert(!mActive, "WCPlayThrough::WCPlayThrough: Can't init while active.");
    ...
    try
    {
        ...
        if(mOutputDeviceIOProcSemaphore == SEMAPHORE_NULL)
        {
            ...
            BGM_Utils::ThrowIfMachError("WCPlayThrough::WCPlayThrough", "semaphore_create", theError);
            
            ThrowIf(mOutputDeviceIOProcSemaphore == SEMAPHORE_NULL,
                    CAException(kAudioHardwareUnspecifiedError),
                    "WCPlayThrough::WCPlayThrough: Could not create semaphore");
        }
    }
    catch (...)
    {
...
void    WCPlayThrough::Activate()
{
    CAMutex::Locker stateLocker(mStateMutex);
    
    if(!mActive)
    {
        DebugMsg("WCPlayThrough::Activate: Activating playthrough");
        
        CreateIOProcIDs();
        
        mActive = true;
        
        // TODO: This code (the next two blocks) should be in WCDeviceControlSync.
        ...
        {
            LogWarning("WCPlayThrough::Activate: Failed to sync device sample rates. Error: %d",
                       e.GetError());
        }
        ...
        {
            LogWarning("WCPlayThrough::Activate: Failed to sync device buffer sizes. Error: %d",
                       e.GetError());
        }
        ...
        if(mRequireBGMDeviceInput)
        {
            ...
            {
                LogWarning("WCPlayThrough::Activate: Playthrough activated with an input device "
                           "other than BGMDevice. This hasn't been tested and is almost "
                           "definitely a bug.");
            }
            BGMAssert(mInputDevice.IsBGMDeviceInstance(),
                      "WCPlayThrough::Activate: !mInputDevice.IsBGMDeviceInstance()");
        }
    }
}

void    WCPlayThrough::Deactivate()
{
    CAMutex::Locker stateLocker(mStateMutex);
    
    if(mActive)
    {
        DebugMsg("WCPlayThrough::Deactivate: Deactivating playthrough");
        
        BGMLogAndSwallowExceptions("WCPlayThrough::Deactivate", [&] {
            Stop();
        });
...
void    WCPlayThrough::AllocateBuffer()
{
...
void    WCPlayThrough::DeallocateBuffer()
{
    // Need to lock the buffer mutexes to make sure the IOProcs aren't accessing it. The order is
    // important here. We always lock them in the same order to prevent deadlocks.
    CAMutex::Locker lockerInput(mBufferInputMutex);
...
// static
void    WCPlayThrough::DisableIOProcStreams(BGMAudioDevice& inDevice,
                                             AudioDeviceIOProcID inIOProcID,
                                             bool inIsInput,
                                             const char* inDeviceName)
{
    BGMLogAndSwallowExceptions("WCPlayThrough::DisableIOProcStreams", [&] {
        ...
        DebugMsg("WCPlayThrough::DisableIOProcStreams: Disabled %u %s stream(s) on the %s device",
                 numStreams, (inIsInput ? "input" : "output"), inDeviceName);
    });
}

void    WCPlayThrough::CreateIOProcIDs()
{
    CAMutex::Locker stateLocker(mStateMutex);
    
    BGMAssert(!mPlayingThrough,
              "WCPlayThrough::CreateIOProcIDs: Tried to create IOProcs when playthrough was already running");
    BGMAssert(mInputDeviceIOProcID == nullptr,
              "WCPlayThrough::CreateIOProcIDs: mInputDeviceIOProcID must be destroyed first.");
    BGMAssert(mOutputDeviceIOProcID == nullptr,
              "WCPlayThrough::CreateIOProcIDs: mOutputDeviceIOProcID must be destroyed first.");
    BGMAssert(CheckIOProcsAreStopped(),
              "WCPlayThrough::CreateIOProcIDs: IOProcs not ready.");
    ...
    {
        DebugMsg("WCPlayThrough::CreateIOProcIDs: Creating IOProcs");
    
        try
        {
            mInputDeviceIOProcID = mInputDevice.CreateIOProcID(&WCPlayThrough::InputDeviceIOProc, this);
        }
        catch(const CAException& e)
        {
            LogWarning("WCPlayThrough::CreateIOProcIDs: Failed to create input IOProc ID. mInputDevice = %d",
        ...
        try
        {
            mOutputDeviceIOProcID = mOutputDevice.CreateIOProcID(&WCPlayThrough::OutputDeviceIOProc, this);
        }
        catch(const CAException& e)
        {
            LogWarning("WCPlayThrough::CreateIOProcIDs: Failed to create output IOProc ID. mOutputDevice = %d",
        ...
        if(mInputDeviceIOProcID == nullptr || mOutputDeviceIOProcID == nullptr)
        {
            ...
            LogError("WCPlayThrough::CreateIOProcIDs: Null IOProc ID returned by CreateIOProcID");
    ...
    {
        LogWarning("WCPlayThrough::CreateIOProcIDs: Failed to create IOProcs.%s%s",
...
void    WCPlayThrough::DestroyIOProcIDs()
{
    CAMutex::Locker stateLocker(mStateMutex);
    ...
    BGMAssert(CheckIOProcsAreStopped(), "WCPlayThrough::DestroyIOProcIDs: IOProcs not ready.");

    DebugMsg("WCPlayThrough::DestroyIOProcIDs: Destroying IOProcs");
        ...
        if(ioProcID != nullptr)
        {
            ...
            catch(const CAException& e)
            {
                if((e.GetError() == kAudioHardwareBadDeviceError) || (e.GetError() == kAudioHardwareBadObjectError))
                {
                    ...
                    DebugMsg("WCPlayThrough::DestroyIOProcIDs: Didn't destroy IOProc ID for %s device because "
...
bool    WCPlayThrough::CheckIOProcsAreStopped() const noexcept
{
    bool statesOK = true;
    
    if(mInputDeviceIOProcState != IOState::Stopped)
    {
        LogWarning("WCPlayThrough::CheckIOProcsAreStopped: Input IOProc not stopped. mInputDeviceIOProcState = %d",
    ...
    {
        LogWarning("WCPlayThrough::CheckIOProcsAreStopped: Output IOProc not stopped. mOutputDeviceIOProcState = %d",
...
void    WCPlayThrough::SetDevices(const BGMAudioDevice* __nullable inInputDevice,
                                   const BGMAudioDevice* __nullable inOutputDevice)
{
    ...
    {
        BGMAssert(wasActive, "WCPlayThrough::SetOutputDevice: wasPlayingThrough && !wasActive");  // Sanity check.
    }
...
void    WCPlayThrough::Start()
{
    CAMutex::Locker stateLocker(mStateMutex);
    
    if(mPlayingThrough)
    {
        DebugMsg("WCPlayThrough::Start: Already started/starting.");
    ...
    {
        LogError("WCPlayThrough::Start: %s %s",
    ...
    catch(const CAException& e)
    {
        ...
        LogError("WCPlayThrough::Start: Failed to start %s device. Error: %d (%s)",
...
OSStatus    WCPlayThrough::WaitForOutputDeviceToStart() noexcept
{
    // Check for errors.
    //
    // Technically we should take the state mutex here, but that could cause deadlocks because
    // WC_Device::StartIO (in BGMDriver) blocks on this function (via XPC). Other WCPlayThrough
    // functions make requests to BGMDriver while holding the state mutex, usually to get/set
    // properties, but the HAL will block those requests until WC_Device::StartIO returns.
    try
    {
        if(!mActive)
        {
            LogError("WCPlayThrough::WaitForOutputDeviceToStart: !mActive");
            return kAudioHardwareNotRunningError;
        }
        
        if(!mOutputDevice.IsAlive())
        {
            LogError("WCPlayThrough::WaitForOutputDeviceToStart: Device not alive");
    ...
    else if(initialState != IOState::Starting)
    {
        ...
        // running.
        LogWarning("WCPlayThrough::WaitForOutputDeviceToStart: Device not starting");
    ...
    DebugMsg("WCPlayThrough::WaitForOutputDeviceToStart: Waiting.");
    ...
    do
    {
        BGMAssert(mOutputDeviceIOProcSemaphore != SEMAPHORE_NULL,
                  "WCPlayThrough::WaitForOutputDeviceToStart: !mOutputDeviceIOProcSemaphore");
    ...
    if(BGMDebugLoggingIsEnabled())
    {
        ...
        DebugMsg("WCPlayThrough::WaitForOutputDeviceToStart: Started %f ms after notification, %f "
    ...
    switch (theError)
    {
        ...
        default:
            BGM_Utils::LogIfMachError("WCPlayThrough::WaitForOutputDeviceToStart",
                                      "semaphore_timedwait",
                                      theError);
...
void    WCPlayThrough::ReleaseThreadsWaitingForOutputToStart()
{
    if(mActive)
    {
...
OSStatus    WCPlayThrough::Stop()
{
    ...
    {
        DebugMsg("WCPlayThrough::Stop: Stopping playthrough");
        ...
        // Wait for the IOProcs to stop themselves. This is so the IOProcs don't get called after the WCPlayThrough instance
        ...
        {
            LogError("WCPlayThrough::Stop: The input IOProc didn't stop itself in time. Stopping "
                     "it from outside of the IO thread.");
            
            BGMLogUnexpectedExceptions("WCPlayThrough::Stop", [&]() {
                mInputDevice.StopIOProc(mInputDeviceIOProcID);
            });
        ...
        {
            LogError("WCPlayThrough::Stop: The output IOProc didn't stop itself in time. Stopping "
                     "it from outside of the IO thread.");
...
void    WCPlayThrough::StopIfIdle()
{
    ...
    // Same guard as Activate() -- WCTapRoute sets mRequireBGMDeviceInput to false for its own
    // WCPlayThrough instance, since its input is a private aggregate tap device, not BGMDevice.
    ...
    // is only called on WCAudioDeviceManager's own BGMDevice-input instances, never on a
    // WCTapRoute's), so this returns rather than guessing at BGMDevice-specific behavior for a
    ...
    DebugMsg("WCPlayThrough::StopIfIdle: BGMDevice is only running IO for BGMApp. "
...
// static
bool    WCPlayThrough::IsRunningSomewhereOtherThanBGMApp(const BGMAudioDevice& inBGMDevice)
{
    ...
    {
        LogWarning("WCPlayThrough::IsRunningSomewhereOtherThanBGMApp: Unexpected property type");
    }
...
// static
OSStatus    WCPlayThrough::InputDeviceIOProc(AudioObjectID           inDevice,
                                              ...
                                              void* __nullable        inClientData)
{
    ...
    WCPlayThrough* const refCon = static_cast<WCPlayThrough*>(inClientData);
    ...
    BGMAssert(state == IOState::Running, "WCPlayThrough::InputDeviceIOProc: Unexpected state");
...
// static
OSStatus    WCPlayThrough::OutputDeviceIOProc(AudioObjectID           inDevice,
                                               const AudioTimeStamp*   inNow,
                                               const AudioBufferList*  inInputData,
                                               const AudioTimeStamp*   inInputTime,
                                               AudioBufferList*        outOutputData,
                                               ...
                                               void* __nullable        inClientData)
{
    ...
    BGMAssert(state == IOState::Running, "WCPlayThrough::OutputDeviceIOProc: Unexpected state");
    ...
    if(didChangeState)
    {
        ...
        BGMAssert(refCon->mLastOutputSampleTime == -1,
                  "WCPlayThrough::OutputDeviceIOProc: mLastOutputSampleTime not reset");
...
// static
inline void WCPlayThrough::FillWithSilence(AudioBufferList* ioBuffer)
{
...
// static
bool    WCPlayThrough::UpdateIOProcState(const char* inCallerName,
                                          WCPlayThroughRTLogger& inRTLogger,
                                          std::atomic<IOState>& inState,
                                          AudioDeviceIOProcID __nullable inIOProcID,
                                          BGMAudioDevice& inDevice,
                                          IOState& outNewState)
{
    BGMAssert(inIOProcID != nullptr, "WCPlayThrough::UpdateIOProcState: !inIOProcID");
    ...
    if(didChangeState)
    {
        BGMAssert(prevState == IOState::Starting, "WCPlayThrough::UpdateIOProcState: ?!");
    ...
    else
    {
        ...
        if(outNewState != IOState::Running)
        {
            ...
            BGMAssert(outNewState == IOState::Stopping,
                      "WCPlayThrough::UpdateIOProcState: Unexpected state: %d",
                      outNewState);
