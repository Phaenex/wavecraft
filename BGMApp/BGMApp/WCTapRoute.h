...
//  used as the input side of one WCPlayThrough instance PER output device (generalized in this
...
//  WCPlayThrough instances sharing the one tap/aggregate device as their common input -- the tap
//  itself, the expensive per-app resource, is created once regardless of how many outputs are
//  attached to it. So the actual real-time audio bridging reuses WCPlayThrough's already-proven
//  ring-buffer/clock-sync engine, once per output; this class is only responsible for the
//  tap/aggregate-device lifecycle and the set of WCPlayThrough instances around it.
//
...
public:
    /*!
     @param inAppBundleID The bundle ID of the app to route. Retained for the lifetime of this
                          object.
     */
    explicit            WCTapRoute(CACFString inAppBundleID);
    ~WCTapRoute();

    WCTapRoute(const WCTapRoute&) = delete;
    WCTapRoute& operator=(const WCTapRoute&) = delete;
    ...
    // FourCharCode-style error constants, the same way WCPlayThrough::kDeviceNotStarting is.
    ...
    /*!
     ...
     @throws CAException If WCPlayThrough fails to activate/start for this device. Only this one
                         ...
                         ambiguous CoreAudio error happened to come back -- see WCTapRoute.mm), or
...
private:
    ...
    struct Output
    {
        ...
        std::unique_ptr<WCPlayThrough>     playThrough;
    };
