...
@implementation WCAudioDeviceManager {
    // This ivar is a pointer so that WCBackgroundMusicDevice's constructor doesn't get called
    // during [WCAudioDeviceManager alloc] when the ivars are initialised. It queries the HAL for
    // BGMDevice's AudioObject ID, which might throw a CAException, most likely because BGMDevice
    // isn't installed.
    //
    // That would be the only way for [WCAudioDeviceManager alloc] to throw a CAException, so we
    ...
    WCDeviceControlSync deviceControlSync;
    WCPlayThrough playThrough;
    WCPlayThrough playThrough_UISounds;
    ...
    WCOutputVolumeMenuItem* __nullable outputVolumeMenuItem;
    WCOutputDeviceMenuSection* __nullable outputDeviceMenuSection;
...
- (instancetype) init {
    if ((self = [super init])) {
        ...
        try {
            bgmDevice = new WCBackgroundMusicDevice;
        } catch (const CAException& e) {
            LogError("WCAudioDeviceManager::init: BGMDevice not found. (%d)", e.GetError());
            self = nil;
            return self;
        }
...
// WCAudioDeviceManager is a singleton and lives for the whole process. We still remove the
// listeners and free bgmDevice here, so deallocating in a test (or if init fails) is safe.
//
// The listener proc's client data is an unretained pointer to the WCAudioDeviceManager instance,
...
- (void) dealloc {
    BGMAssert(NSClassFromString(@"XCTestCase") != nil,  // Returns nil unless running in a test
              "WCAudioDeviceManager is not safe to destroy (except in unit tests).");
...
- (void) setOutputVolumeMenuItem:(WCOutputVolumeMenuItem*)item {
    outputVolumeMenuItem = item;
}

- (void) setOutputDeviceMenuSection:(WCOutputDeviceMenuSection*)menuSection {
...
- (NSError* __nullable) setBGMDeviceAsOSDefault {
    try {
        ...
        BGMLogExceptionIn("WCAudioDeviceManager::setBGMDeviceAsOSDefault", e);
...
- (NSError* __nullable) unsetBGMDeviceAsOSDefault {
    ...
    WCBackgroundMusicDevice* bgmDeviceCopy;
    ...
    try {
        ...
        BGMLogExceptionIn("WCAudioDeviceManager::unsetBGMDeviceAsOSDefault", e);
        return [NSError errorWithDomain:@kBGMAppBundleID code:e.GetError() userInfo:nil];
    }
...
- (NSError* __nullable) setOutputDeviceWithIDImpl:(AudioObjectID)newDeviceID
                                  ...
                                  revertOnFailure:(BOOL)revertOnFailure {
    DebugMsg("WCAudioDeviceManager::setOutputDeviceWithIDImpl: Setting output device. newDeviceID=%u",
             newDeviceID);
...
- (void) setOutputDeviceWithIDImpl:(AudioObjectID)newDeviceID
    ...
    DebugMsg("WCAudioDeviceManager::setOutputDeviceWithIDImpl: Set output device to %s (%d)",
...
- (void) setDataSource:(UInt32)dataSourceID device:(BGMAudioDevice&)device {
    BGMLogAndSwallowExceptions("WCAudioDeviceManager::setDataSource", ([&] {
        AudioObjectPropertyScope scope = kAudioObjectPropertyScopeOutput;
        UInt32 channel = 0;

        if (device.DataSourceControlIsSettable(scope, channel)) {
            DebugMsg("WCAudioDeviceManager::setOutputDeviceWithID: Setting dataSourceID=%u",
                     dataSourceID);
...
- (NSError*) failedToSetOutputDevice:(AudioDeviceID)deviceID
    ...
    NSLog(@"WCAudioDeviceManager::failedToSetOutputDevice: Couldn't set device with ID %u as output device. "
          "%s%d. %@",
          deviceID,
          "Error: ", errorCode,
    ...
    if (revertTo) {
        ...
        NSLog(@"WCAudioDeviceManager::failedToSetOutputDevice: Failed to revert to the previous device.");
    }
...
- (OSStatus) startPlayThroughSync:(BOOL)forUISoundsDevice {
    ...
    // makes to BGMDevice block in the HAL if another thread is in WC_Device::StartIO.
    //
    // Since WC_Device::StartIO calls this method (via XPC), waiting for setOutputDeviceWithID to release
    ...
    @try {
        ...
        if (!isBigSur && gotLock) {
            WCPlayThrough& pt = (forUISoundsDevice ? playThrough_UISounds : playThrough);
            ...
            //       cause deadlocks.
            BGMLogAndSwallowExceptionsMsg("WCAudioDeviceManager::startPlayThroughSync",
                                          "Starting playthrough", [&] {
                pt.Start();
            });

            err = pt.WaitForOutputDeviceToStart();
            BGMAssert(err != WCPlayThrough::kDeviceNotStarting, "Playthrough didn't start");
        } else {
            LogWarning("WCAudioDeviceManager::startPlayThroughSync: Didn't get state lock. Returning "
            ...
            dispatch_async(BGMGetDispatchQueue_PriorityUserInteractive(), ^{
                @try {
                    ...
                    BGMLogAndSwallowExceptions("WCAudioDeviceManager::startPlayThroughSync", [&] {
                        pt.StopIfIdle();
                    });
                } @finally {
                    [stateLock unlock];
...
//
// Listeners are registered here, rather than by WCPlayThrough, so the listener proc's client data
// pointer always remains valid. WCPlayThrough instances currently only get destroyed when BGMApp
...
- (void) registerBGMDeviceListeners {
    ...
    auto addListeners = [&](AudioObjectID deviceID) {
        BGMLogAndSwallowExceptions("WCAudioDeviceManager::registerBGMDeviceListeners", [&] {
...
- (void) removeBGMDeviceListeners {
    ...
    auto removeListeners = [&](AudioObjectID deviceID) {
        ...
        BGMLogAndSwallowExceptions("WCAudioDeviceManager::removeBGMDeviceListeners", [&] {
            BGMAudioDevice device(deviceID);
...
- (void) handleBGMDeviceNotification:(AudioObjectID)inDeviceID
    ...
    BGMAssert(inDeviceID == bgmDevice->GetObjectID() ||
              ...
              "WCAudioDeviceManager::handleBGMDeviceNotification: "
...
// Returns the WCPlayThrough for the given BGMDevice instance.
- (WCPlayThrough&) playThroughForDeviceID:(AudioObjectID)inDeviceID {
...
//
// Either this notification or WCXPCListener::startPlayThroughSyncWithReply will start
...
- (void) handleDeviceIsRunningSomewhere:(AudioObjectID)inDeviceID {
    DebugMsg("WCAudioDeviceManager::handleDeviceIsRunningSomewhere: Got notification for "
             "device %u",
             inDeviceID);

    // This is dispatched because it can block and WCXPCListener::startPlayThroughSyncWithReply
    ...
    dispatch_async(BGMGetDispatchQueue_PriorityUserInteractive(), ^{
        @try {
            ...
            WCPlayThrough& pt = [self playThroughForDeviceID:inDeviceID];
            ...
            DebugMsg("WCAudioDeviceManager::handleDeviceIsRunningSomewhere: "
...
// WC_Clients). That's enough time for BGMDriver to update
...
- (void) handleDeviceIsRunningSomewhereOtherThanBGMApp:(AudioObjectID)inDeviceID {
    DebugMsg("WCAudioDeviceManager::handleDeviceIsRunningSomewhereOtherThanBGMApp: "
...
- (void) sendOutputDeviceToBGMXPCHelper {
    ...
    if (connection)
    {
        id<WCXPCHelperXPCProtocol> helperProxy =
                [connection remoteObjectProxyWithErrorHandler:^(NSError* error) {
                    // We could wait a bit and try again, but it isn't that important.
                    NSLog(@"WCAudioDeviceManager::sendOutputDeviceToBGMXPCHelper: Connection"
                           "error: %@", error);
                }];
                                  ...
                                  void* __nullable inClientData)
{
    ...
    WCAudioDeviceManager* manager = (__bridge WCAudioDeviceManager*)inClientData;
