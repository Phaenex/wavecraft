...
@implementation WCPreferredOutputDevices {
    NSRecursiveLock* _stateLock;

    // Used to change BGMApp's output device.
    WCAudioDeviceManager* _devices;

    // User settings and data.
    WCUserDefaults* _userDefaults;
...
- (instancetype) initWithDevices:(WCAudioDeviceManager*)devices
                    userDefaults:(WCUserDefaults*)userDefaults {
    if ((self = [super init])) {
        ...
        DebugMsg("WCPreferredOutputDevices::initWithDevices: Preferred devices: %s",
...
- (NSArray<NSString*>*) readPreferredDevices {
    ...
    if (error || !data || !settings) {
        ...
        LogWarning("WCPreferredOutputDevices::readPreferredDevices: Couldn't read %s. "
        ...
        // output).
        LogWarning("WCPreferredOutputDevices::readPreferredDevices: No preferred devices in %s",
                   settings.debugDescription.UTF8String);
    } else if (!settings[@"preferred devices"][@"output"]) {
        // The Plist doesn't include the list of preferred output devices.
        LogWarning("WCPreferredOutputDevices::readPreferredDevices: "
...
- (NSArray<NSString*>*) preferredDeviceUIDsFrom:(NSArray<NSDictionary*>*)deviceInfos
    ...
    for (NSDictionary* deviceInfo in deviceInfos) {
        ...
        if (![deviceInfo[@"uid"] isKindOfClass:NSString.class]) {
            LogWarning("WCPreferredOutputDevices::preferredDeviceUIDsFrom: No UID in %s",
...
- (NSArray<NSString*>*) addNextStoredPreferredDevice:(int*)storedPreferredDeviceIdx
    ...
    if (storedPreferredDevice) {
        DebugMsg("WCPreferredOutputDevices::addNextStoredPreferredDevice: "
...
- (void) listenForDevicesAddedOrRemoved {
    ...
    WCPreferredOutputDevices* __weak weakSelf = self;
...
- (void) connectedDeviceListChanged {
    @try {
        ...
        if (preferredDevice == kAudioObjectUnknown) {
            LogWarning("WCPreferredOutputDevices::connectedDeviceListChanged: "
                       "No preferred device found.");
        } else if (_devices.outputDevice.GetObjectID() == preferredDevice) {
            DebugMsg("WCPreferredOutputDevices::connectedDeviceListChanged: "
            ...
            if (error) {
                ...
                LogError("WCPreferredOutputDevices::connectedDeviceListChanged: "
...
- (AudioObjectID) findPreferredDevice {
    ...
    if (preferredDevice == kAudioObjectUnknown) {
        ...
        DebugMsg("WCPreferredOutputDevices::findPreferredDevice: Checking Plist and stored list.");
    ...
    BGM_Utils::LogAndSwallowExceptions(BGMDbgArgs, [&] {
        if (preferredDevice == kAudioObjectUnknown && [self isCurrentOutputDeviceConnected]) {
            ...
            DebugMsg("WCPreferredOutputDevices::findPreferredDevice: "
    ...
    if (preferredDevice == kAudioObjectUnknown) {
        ...
        DebugMsg("WCPreferredOutputDevices::findPreferredDevice: Choosing an arbitrary device.");
        BGM_Utils::LogAndSwallowExceptions(BGMDbgArgs, [&] {
    ...
    for (int position = 0; position < _preferredDeviceUIDs.count; position++) {
        ...
        for (UInt32 i = 0; i < numDevices; i++) {
            ...
            if ([connectedDeviceUID isEqualToString:_preferredDeviceUIDs[position]]) {
                ...
                DebugMsg("WCPreferredOutputDevices::findPreferredDeviceInDerivedList: "
...
- (bool) isCurrentOutputDeviceConnected {
    if (_devices.outputDevice.GetObjectID() == kAudioObjectUnknown) {
        DebugMsg("WCPreferredOutputDevices::isCurrentOutputDeviceConnected: "
    ...
    @try {
        ...
        BGM_Utils::LogAndSwallowExceptions(BGMDbgArgs, [&] {
            ...
            if (outputDeviceUID) {
                ...
                DebugMsg("WCPreferredOutputDevices::userChangedOutputDeviceTo: "
                         "Preferred devices: %s",
                         _preferredDeviceUIDs.debugDescription.UTF8String);
                ...
                LogWarning("WCPreferredOutputDevices::userChangedOutputDeviceTo: "
