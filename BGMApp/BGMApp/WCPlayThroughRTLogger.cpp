...
// Self Include
#include "WCPlayThroughRTLogger.h"
...
WCPlayThroughRTLogger::WCPlayThroughRTLogger()
{
    ...
    mLoggingThread = std::thread(&WCPlayThroughRTLogger::LoggingThreadEntry, this);
}

// static
semaphore_t WCPlayThroughRTLogger::CreateSemaphore()
{
    ...
    BGM_Utils::ThrowIfMachError("WCPlayThroughRTLogger::CreateSemaphore",
                                "semaphore_create",
                                error);
    ThrowIf(semaphore == SEMAPHORE_NULL,
            CAException(kAudioHardwareUnspecifiedError),
            "WCPlayThroughRTLogger::CreateSemaphore: Failed to create semaphore");
...
WCPlayThroughRTLogger::~WCPlayThroughRTLogger()
{
    ...
    BGM_Utils::LogIfMachError("WCPlayThroughRTLogger::~WCPlayThroughRTLogger",
                              "semaphore_signal",
                              error);
...
void WCPlayThroughRTLogger::LogReleasingWaitingThreads()
{
...
void WCPlayThroughRTLogger::LogIfMachError_ReleaseWaitingThreadsSignal(mach_error_t inError)
{
...
void WCPlayThroughRTLogger::LogIfDroppedFrames(Float64 inFirstInputSampleTime,
...
void WCPlayThroughRTLogger::LogNoSamplesReady(CARingBuffer::SampleTime inLastInputSampleTime,
...
void WCPlayThroughRTLogger::LogExceptionStoppingIOProc(const char* inCallerName,
                                                        OSStatus inError,
                                                        bool inErrorKnown)
{
...
void WCPlayThroughRTLogger::LogUnexpectedIOStateAfterStopping(const char* inCallerName,
                                                               int inIOState)
{
...
void WCPlayThroughRTLogger::LogRingBufferUnavailable(const char* inCallerName, bool inGotLock)
{
...
void WCPlayThroughRTLogger::LogIfRingBufferError(CARingBufferError inError,
...
void WCPlayThroughRTLogger::LogAsync(T& inMessageData, F&& inStoreMessageData)
{
...
void WCPlayThroughRTLogger::LogSync_Warning(const char* inFormat, ...)
{
    va_list args;
...
void WCPlayThroughRTLogger::LogSync_Error(const char* inFormat, ...)
{
    va_list args;
    va_start(args, inFormat);
...
void WCPlayThroughRTLogger::WakeLoggingThread()
{
    kern_return_t error = semaphore_signal(mWakeUpLoggingThreadSemaphore);
...
void WCPlayThroughRTLogger::LogMessages()
{
...
void WCPlayThroughRTLogger::LogSync_ReleasingWaitingThreads()
{
    if(mLogReleasingWaitingThreadsMsg)
    {
        LogSync_Debug("WCPlayThrough::ReleaseThreadsWaitingForOutputToStart: "
...
void WCPlayThroughRTLogger::LogSync_ReleaseWaitingThreadsSignalError()
{
    if(mReleaseWaitingThreadsSignalError != KERN_SUCCESS)
    {
        BGM_Utils::LogIfMachError("WCPlayThrough::ReleaseThreadsWaitingForOutputToStart",
...
void WCPlayThroughRTLogger::LogSync_DroppedFrames()
{
    if(mDroppedFrames.shouldLogMessage)
    {
        LogSync_Debug("WCPlayThrough::OutputDeviceIOProc: "
...
void WCPlayThroughRTLogger::LogSync_NoSamplesReady()
{
...
void WCPlayThroughRTLogger::LogSync_UnexpectedIOStateAfterStopping()
{
    if(mUnexpectedIOStateAfterStopping.shouldLogMessage)
    {
        LogSync_Warning("WCPlayThrough::UpdateIOProcState: "
...
void WCPlayThroughRTLogger::LogSync_RingBufferUnavailable()
{
    if(mRingBufferUnavailable.shouldLogMessage)
    {
        LogSync_Warning("WCPlayThrough::%s: Ring buffer unavailable. %s",
...
void WCPlayThroughRTLogger::LogSync_RingBufferError(
        ...
        const char* inMethodName)
{
    ...
    switch(error)
    {
        ...
        case kCARingBufferError_CPUOverload:
            ...
            LogSync_Warning("WCPlayThrough::%s: Ring buffer error: "
        ...
        default:
            ...
            LogSync_Error("WCPlayThrough::%s: Ring buffer error: %s (%d)",
                          inMethodName,
...
// static
void* __nullable WCPlayThroughRTLogger::LoggingThreadEntry(WCPlayThroughRTLogger* inRefCon)
{
    DebugMsg("WCPlayThroughRTLogger::IOProcLoggingThreadEntry: "
    ...
    while(!inRefCon->mLoggingThreadShouldExit)
    {
        ...
        BGM_Utils::LogIfMachError("WCPlayThroughRTLogger::IOProcLoggingThreadEntry",
                                  "semaphore_wait",
                                  error);
    }

    DebugMsg("WCPlayThroughRTLogger::IOProcLoggingThreadEntry: IOProc logging thread exiting");
...
bool WCPlayThroughRTLogger::WaitUntilLoggerThreadIdle()
{
    int msWaited = 0;
