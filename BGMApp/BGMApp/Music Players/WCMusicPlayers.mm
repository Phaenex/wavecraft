...
#import "WCSwinsian.h"
#import "WCMusic.h"
#import "WCGooglePlayMusicDesktopPlayer.h"


#pragma clang assume_nonnull begin

@implementation WCMusicPlayers {
...
- (instancetype) initWithAudioDevices:(WCAudioDeviceManager*)devices
    ...
    // it to this array.
    NSArray<Class<WCMusicPlayer>>* mpClasses = @[ [WCVOX class],
                                                   [WCVLC class],
                                                   [WCSpotify class],
                                                   [WCiTunes class],
                                                   [WCDecibel class],
    ...
    if (@available(macOS 10.10, *)) {
        mpClasses = [mpClasses arrayByAddingObject:[WCGooglePlayMusicDesktopPlayer class]];
    }

    return [self initWithAudioDevices:devices
                 defaultMusicPlayerID:[WCiTunes sharedMusicPlayerID]
...
- (instancetype) initWithAudioDevices:(WCAudioDeviceManager*)devices
                 defaultMusicPlayerID:(NSUUID*)defaultMusicPlayerID
                   musicPlayerClasses:(NSArray<Class<WCMusicPlayer>>*)musicPlayerClasses
                         userDefaults:(WCUserDefaults*)defaults {
    if ((self = [super init])) {
        ...
        for (Class<WCMusicPlayer> musicPlayerClass in musicPlayerClasses) {
            NSArray<id<WCMusicPlayer>>* instances =
        ...
        NSAssert(_selectedMusicPlayer, @"WCMusicPlayers::initWithAudioDevices: !_selectedMusicPlayer");
    }
...
- (void) initSelectedMusicPlayerFromUserDefaults {
    ...
    if (selectedMusicPlayerIDStr) {
        ...
        NSAssert(selectedMusicPlayerID,
                 @"WCMusicPlayers::initSelectedMusicPlayerFromUserDefaults: !selectedMusicPlayerID");
    }
...
#if DEBUG
        DebugMsg("WCMusicPlayers::initSelectedMusicPlayerFromUserDefaults: %s selectedMusicPlayerIDStr=%s",
...
- (void) initSelectedMusicPlayerFromBGMDevice {
    ...
    DebugMsg("WCMusicPlayers::initSelectedMusicPlayerFromBGMDevice: "
    ...
    if (bundleID && ![bundleID isEqualToString:@""]) {
        ...
        NSArray<id<WCMusicPlayer>>* matchingMusicPlayers = @[ ];
        
        for (id<WCMusicPlayer> musicPlayer in _musicPlayers) {
            NSString* bundleIDNN = bundleID;
            if ([musicPlayer.bundleID isEqualToString:bundleIDNN]) {
                DebugMsg("WCMusicPlayers::initSelectedMusicPlayerFromBGMDevice: Bundle ID on BGMDevice matches %s",
...
- (id<WCMusicPlayer>) selectedMusicPlayer {
    return _selectedMusicPlayer;
}

- (void) setSelectedMusicPlayer:(id<WCMusicPlayer>)newSelectedMusicPlayer {
    ...
    NSAssert(self.selectedMusicPlayer == newSelectedMusicPlayer,
             @"WCMusicPlayers::setSelectedMusicPlayer: selectedMusicPlayer wasn't set to the object expected");
}

- (BOOL) setSelectedMusicPlayerByID:(NSUUID*)newSelectedMusicPlayerID {
    id<WCMusicPlayer> __nullable newSelectedMusicPlayer = nil;
    
    // Find the music player with the given ID, if there is one.
    for (id<WCMusicPlayer> musicPlayer in _musicPlayers) {
        if ([musicPlayer.musicPlayerID isEqual:newSelectedMusicPlayerID]) {
            NSAssert(!newSelectedMusicPlayer, @"WCMusicPlayers::setSelectedMusicPlayerByID: Non-unique musicPlayerID");
    ...
    if (newSelectedMusicPlayer) {
        // (Use setSelectedMusicPlayerImpl to avoid setSelectedMusicPlayer being called in init.)
        id<WCMusicPlayer> newPlayerNN = newSelectedMusicPlayer;
...
- (void) setSelectedMusicPlayerImpl:(id<WCMusicPlayer>)newSelectedMusicPlayer {
    NSAssert([_musicPlayers containsObject:newSelectedMusicPlayer],
             @"WCMusicPlayers::setSelectedMusicPlayerImpl: Only the music players in the musicPlayers array can be selected. "
              "newSelectedMusicPlayer=%@",
             newSelectedMusicPlayer.name);

    if (_selectedMusicPlayer == newSelectedMusicPlayer) {
        DebugMsg("WCMusicPlayers::setSelectedMusicPlayerImpl: %s is already the selected music "
                 "player.",
    ...
    DebugMsg("WCMusicPlayers::setSelectedMusicPlayerImpl: Set selected music player to %s",
             _selectedMusicPlayer.name.UTF8String);
...
- (void) updateBGMDeviceMusicPlayerProperties {
    ...
    NSAssert(self.selectedMusicPlayer.pid || self.selectedMusicPlayer.bundleID,
             @"WCMusicPlayers::updateBGMDeviceMusicPlayerProperties: Music player has neither bundle ID nor PID");
