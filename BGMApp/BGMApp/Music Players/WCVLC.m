...
@implementation WCVLC {
    WCScriptingBridge* scriptingBridge;
}

- (instancetype) init {
    if ((self = [super initWithMusicPlayerID:[WCMusicPlayerBase makeID:@"5226F4B9-C740-4045-A273-4B8EABC0E8FC"]
                                        name:@"VLC"
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("WCVLC::pause: Pausing VLC");
        [WCVLC togglePlay];
    }
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("WCVLC::unpause: Unpausing VLC");
        [WCVLC togglePlay];
    }
