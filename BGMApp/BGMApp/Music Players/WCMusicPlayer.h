...
//
//  To add support for a music player, create a class that implements the WCMusicPlayer protocol
//  and add it to initWithAudioDevices in WCMusicPlayers.mm.
//
//  You'll probably want to subclass WCMusicPlayerBase and, if the music player supports
//  AppleScript, use WCScriptingBridge. Your class might need to override the icon method if the
//  default implementation from WCMusicPlayerBase doesn't work.
//
//  WCSpotify will probably be the most useful example to follow, but they're all pretty
...
@protocol WCMusicPlayer <NSObject>
...
// Most classes don't need to override the default implementation from WCMusicPlayerBase.
//
...
+ (NSArray<id<WCMusicPlayer>>*) createInstancesWithDefaults:(WCUserDefaults*)userDefaults;
...
//       BGMDevice of changes. It might be easiest to have WCMusicPlayers to observe this property,
//       on the selected music player, with KVO and update BGMDevice when it changes. Or
//       WCMusicPlayers could pass a pointer to itself to createInstancesWithDefaults.
...
// WCMusicPlayer::unpause is called. Normally because the user was playing a song and they or
// BGMApp paused it.
...
@interface WCMusicPlayerBase : NSObject
...
// WCMusicPlayer default implementations
+ (NSArray<id<WCMusicPlayer>>*) createInstancesWithDefaults:(WCUserDefaults*)userDefaults;
@property (readonly) NSImage* __nullable icon;
@property (readonly) NSUUID* musicPlayerID;
@property (readonly) NSString* name;
