...
//
//  WCGooglePlayMusicDesktopPlayerConnection.m
//  BGMApp
//
...
// Self Include
#import "WCGooglePlayMusicDesktopPlayerConnection.h"
...
@implementation WCGooglePlayMusicDesktopPlayerConnection {
...
- (instancetype) initWithUserDefaults:(WCUserDefaults*)defaults
...
- (void) connectWithRetries:(int)retries {
    ...
    if (permanentAuthCode) {
        DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::connectWithRetries: "
                 "Connecting with auth code");

        NSString* __nullable percentEncodedCode =
                [WCGooglePlayMusicDesktopPlayerConnection
...
- (void) disconnect {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::disconnect: Disconnecting");
...
- (void) sendAuthCode:(NSString*)authCode {
    ...
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::sendAuthCode: Sending GPMDP auth code");
    ...
    NSString* __nullable percentEncodedCode =
            [WCGooglePlayMusicDesktopPlayerConnection toPercentEncoded:authCode];
...
- (void) sendPermanentAuthCode {
    ...
    if (code) {
        // Don't log the code itself just in case it could be a security problem.
        DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::sendPermanentAuthCode: "
                 "Sending GPMDP permanent auth code");

        // Percent-encode it just in case something it includes could be executed as Javascript.
        NSString* __nullable percentEncodedCode =
                [WCGooglePlayMusicDesktopPlayerConnection toPercentEncoded:BGMNN(code)];
        ...
        NSLog(@"WCGooglePlayMusicDesktopPlayerConnection::sendPermanentAuthCode: No code to send");
    }
}
...
- (void) handleLogMessage:(NSString*)message {
    ...
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::userContentController: %s",
             message.UTF8String);
#endif
}
...
- (void) handleConnectionError {
    if (connectionRetries > 0) {
        DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handleConnectionError: "
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)),
                       ...
                       ^{
                           ...
                           if (connectionRetries > 0) {
                               DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::"
                                        "handleConnectionError: Retrying");
                               [self connectWithRetries:(connectionRetries - 1)];
                           }
                       });
    } else {
        NSLog(@"WCGooglePlayMusicDesktopPlayerConnection::handleConnectionError: "
...
- (void) handleAPIVersionMessage:(NSDictionary*)body {
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handleAPIVersionMessage: Response: %s",
    ...
    if (versionParts.count > 0) {
        ...
        DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handleAPIVersionMessage: "
...
- (void) handleConnectMessage:(NSDictionary*)body {
    ...
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handleConnectMessage: Received response");
...
- (void) handlePlayStateMessage:(NSDictionary*)body {
    (void)body;
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handlePlayStateMessage: Response: %s",
             [NSString stringWithFormat:@"%@", body].UTF8String);
...
- (void) handleResultMessage:(NSDictionary*)body {
    DebugMsg("WCGooglePlayMusicDesktopPlayerConnection::handleResultMessage: Response: %s",
             [NSString stringWithFormat:@"%@", body].UTF8String);
