...
+ (NSUUID*) sharedMusicPlayerID {
    ...
    NSAssert(musicPlayerID, @"WCiTunes::sharedMusicPlayerID: !musicPlayerID");
    return (NSUUID*)musicPlayerID;
}

- (instancetype) init {
    if ((self = [super initWithMusicPlayerID:[WCiTunes sharedMusicPlayerID]
                                        name:@"iTunes"
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("WCiTunes::pause: Pausing iTunes");
        [self.iTunes pause];
    }
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("WCiTunes::unpause: Unpausing iTunes");
