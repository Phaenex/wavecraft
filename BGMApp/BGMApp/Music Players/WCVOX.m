...
@implementation WCVOX {
    WCScriptingBridge* scriptingBridge;
}

- (instancetype) init {
    if ((self = [super initWithMusicPlayerID:[WCMusicPlayerBase makeID:@"26498C5D-C18B-4689-8B41-9DA91A78FFAD"]
                                        name:@"VOX"
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("WCVOX::pause: Pausing VOX");
        [self.vox pause];
    }
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("WCVOX::unpause: Unpausing VOX");
        [self.vox playpause];
    }
