...
@implementation WCScriptingBridge {
    id<WCMusicPlayer> __weak _musicPlayer;
    WCAppWatcher* appWatcher;
}

@synthesize application = _application;

- (instancetype) initWithMusicPlayer:(id<WCMusicPlayer>)musicPlayer {
    if ((self = [super init])) {
...
- (void) initApplication {
    ...
    WCScriptingBridge* __weak weakSelf = self;

    void (^createSBApplication)(void) = ^{
        WCScriptingBridge* strongSelf = weakSelf;
    ...
    appWatcher =
        [[WCAppWatcher alloc] initWithBundleID:bundleID
                                    appLaunched:^{
                                        DebugMsg("WCScriptingBridge::initApplication: %s launched",
                                                 bundleID.UTF8String);
                                        createSBApplication();
                                  ...
                                  appTerminated:^{
                                      WCScriptingBridge* strongSelf = weakSelf;
                                      DebugMsg("WCScriptingBridge::initApplication: %s terminated",
                                               bundleID.UTF8String);
...
- (void) ensurePermission {
    ...
    if (@available(macOS 10.14, *)) {
        id<WCMusicPlayer> musicPlayer = _musicPlayer;

        if (!musicPlayer.selected) {
            DebugMsg("WCScriptingBridge::ensurePermission: %s not selected. Nothing to do.",
                     musicPlayer.name.UTF8String);
            return;
        }

        if (!musicPlayer.running) {
            DebugMsg("WCScriptingBridge::ensurePermission: %s not running. Nothing to do.",
                     musicPlayer.name.UTF8String);
            return;
        }
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            DebugMsg("WCScriptingBridge::ensurePermission: "
            ...
            if (status != noErr) {
                ...
                NSLog(@"WCScriptingBridge::ensurePermission: Permission denied for %@. status=%d",
                      musicPlayer.name,
                      status);
            }
        });
    } else {
        DebugMsg("WCScriptingBridge::ensurePermission: Not macOS 10.14+. Nothing to do.");
    }
...
#pragma clang diagnostic pop
    ...
    DebugMsg("WCScriptingBridge::eventDidFail: Apple event sent to %s failed. %s",
