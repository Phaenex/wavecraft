...
//
//  WCGooglePlayMusicDesktopPlayer.m
//  BGMApp
//
...
@implementation WCGooglePlayMusicDesktopPlayer {
    WCUserDefaults* userDefaults;
    WCGooglePlayMusicDesktopPlayerConnection* connection;
...
- (instancetype) initWithUserDefaults:(WCUserDefaults*)defaults {
    ...
    // command line tool).
    NSUUID* playerID = [WCMusicPlayerBase makeID:@"FCDCC01F-4BF1-4AD2-BE3E-6B7659A90A3F"];
    if ((self = [super initWithMusicPlayerID:playerID
        ...
        WCGooglePlayMusicDesktopPlayer* __weak weakSelf = self;

        connection = [[WCGooglePlayMusicDesktopPlayerConnection alloc]
                initWithUserDefaults:userDefaults
                 authRequiredHandler:^{
                     WCGooglePlayMusicDesktopPlayer* strongSelf = weakSelf;
...
- (void) gpmdpWasLaunched {
    if (self.selected) {
        ...
        DebugMsg("WCGooglePlayMusicDesktopPlayer::gpmdpWasLaunched: GPMDP launched. Connecting");
...
- (void) gpmdpWasTerminated {
    if (self.selected) {
        ...
        DebugMsg("WCGooglePlayMusicDesktopPlayer::gpmdpWasTerminated: GPMDP has been closed.");
...
- (NSString* __nullable) requestAuthCodeFromUser {
    if (showingAuthCodeDialog) {
        DebugMsg("WCGooglePlayMusicDesktopPlayer::requestAuthCodeFromUser: "
...
- (NSString* __nullable) showAuthCodeDialog {
    ...
    if (buttonPressed == NSAlertFirstButtonReturn) {
        ...
        DebugMsg("WCGooglePlayMusicDesktopPlayer::showAuthCodeDialog: Got auth code: <private>");
        return input.stringValue;
    } else {
        DebugMsg("WCGooglePlayMusicDesktopPlayer::showAuthCodeDialog: "
...
- (void) showErrorDialog:(NSString*)errorMsg troubleshootingMsg:(NSString*)troubleshootingMsg {
    if (!self.running) {
        ...
        DebugMsg("WCGooglePlayMusicDesktopPlayer::showErrorDialog: Not running");
        return;
    }
...
- (BOOL) isRunning {
    ...
    // callbacks because WCAutoPauseMenuItem calls this method when it's notified by its own
    // instance of WCAppWatcher. If WCAutoPauseMenuItem got notified first, the flag wouldn't be
    // updated in time.
    //
    // At some point we might want to try to avoid this by making the WCMusicPlayers' running
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("WCGooglePlayMusicDesktopPlayer::pause: Pausing Google Play Music Desktop "
                 "Player");
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("WCGooglePlayMusicDesktopPlayer::unpause: Unpausing Google Play Music Desktop "
                 "Player");
        [connection playPause];
    }
