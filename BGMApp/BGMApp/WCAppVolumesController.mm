...
@implementation WCAppVolumesController {
    // The App Volumes UI.
    WCAppVolumes* appVolumes;
...
- (id) initWithYourAppsStack:(NSStackView*)yourAppsStack
    ...
    if ((self = [super init])) {
        ...
        appVolumes = [[WCAppVolumes alloc] initWithController:self
...
// on every band, the driver's default -- see WC_AppEQ's default-constructed gains).
- (NSArray<NSNumber*>* __nullable) getEQBandGainsForApp:(NSRunningApplication*)app
