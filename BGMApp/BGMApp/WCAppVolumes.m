...
// (lowest frequency first) as WC_AppEQ::kBandCenterFreqs in WC_Biquad.h -- there's no shared
...
// Shared by WCAVM_PanSlider/WCAVM_EQBandSlider's live-update paths and
// WCAppVolumes::insertRowForApp:... to find the show-more-controls button so its
// non-default-controls highlight can be kept in sync -- see
// WCAVM_ShowMoreControlsButton::bgm_syncHighlightForCurrentControls.
static WCAVM_ShowMoreControlsButton* __nullable BGM_FindShowMoreControlsButton(NSView* siblingContainer) {
    for (NSView* view in siblingContainer.subviews) {
        if ([view isKindOfClass:[WCAVM_ShowMoreControlsButton class]]) {
            return (WCAVM_ShowMoreControlsButton*)view;
        }
    }
    return nil;
}
...
@implementation WCAppVolumes {
    WCAppVolumesController* controller;
...
- (void) insertRowForApp:(NSRunningApplication*)app
    ...
    if (!copiedRowView) {
        DebugMsg("WCAppVolumes::insertRowForApp: Failed to copy the app volume row template");
        return;
    }
    ...
    for (NSView* subview in rowView.subviews) {
        if ([subview conformsToProtocol:@protocol(WCAppVolumeMenuItemSubview)]) {
            [(NSView<WCAppVolumeMenuItemSubview>*)subview setUpWithApp:app
                                                                  context:self
    ...
    // from WCAVM_ShowMoreControlsButton::setUpWithApp: itself, since siblings don't have their
...
#pragma clang diagnostic pop
    ...
    NSView* row = [WCMainPanelContentView rowContainerWithControl:rowView height:kBGMMainPanelRowHeight];
...
- (BGMAppVolumeAndPan) getVolumeAndPanForApp:(NSRunningApplication*)app {
    ...
    for (NSView* subview in rowView.subviews) {
        // Get the volume.
        if ([subview isKindOfClass:[WCAVM_VolumeSlider class]]) {
            result.volume = [(WCAVM_VolumeSlider*)subview intValue];
        }

        // Get the pan position.
        if ([subview isKindOfClass:[WCAVM_PanSlider class]]) {
            result.pan = [(WCAVM_PanSlider*)subview intValue];
        }
    }
...
- (void) setVolumeAndPan:(BGMAppVolumeAndPan)volumeAndPan forApp:(NSRunningApplication*)app {
    ...
    for (NSView* subview in rowView.subviews) {
        // Set the volume.
        if (volumeAndPan.volume != -1 && [subview isKindOfClass:[WCAVM_VolumeSlider class]]) {
            [(WCAVM_VolumeSlider*)subview setRelativeVolume:volumeAndPan.volume];
        }

        // Set the pan position.
        if (volumeAndPan.pan != kAppPanNoValue && [subview isKindOfClass:[WCAVM_PanSlider class]]) {
            [(WCAVM_PanSlider*)subview setPanPosition:volumeAndPan.pan];
        }
    }
}
...
- (void) setVolumeOfRow:(NSView*)rowView relativeVolume:(int)volume panPosition:(int)pan {
    for (NSView* subview in rowView.subviews) {
        if (volume != -1 && [subview isKindOfClass:[WCAVM_VolumeSlider class]]) {
            [(WCAVM_VolumeSlider*)subview setRelativeVolume:volume];
        }

        if (pan != kAppPanNoValue && [subview isKindOfClass:[WCAVM_PanSlider class]]) {
            [(WCAVM_PanSlider*)subview setPanPosition:pan];
        }
    }
}
...
- (void) setEQBandGainsOfRow:(NSView*)rowView gainsDB:(NSArray<NSNumber*>*)gainsDB {
    for (NSView* subview in rowView.subviews) {
        if ([subview isKindOfClass:[WCAVM_EQBandSlider class]]) {
            NSInteger band = ((WCAVM_EQBandSlider*)subview).tag;
            if (band >= 0 && (NSUInteger)band < gainsDB.count) {
                [(WCAVM_EQBandSlider*)subview setGainDB:gainsDB[(NSUInteger)band].floatValue];
            }
        }
    }
}
...
- (void) showHideExtraControls:(WCAVM_ShowMoreControlsButton*)button {
    ...
    if (allSubviewsShowing) {
        // Hide extra controls.
        DebugMsg("WCAppVolumes::showHideExtraControls: Hiding extra controls");
        ...
        // Show extra controls.
        DebugMsg("WCAppVolumes::showHideExtraControls: Showing extra controls");
...
// rowView's wrapping row container (see WCMainPanelContentView.rowContainerWithControl:height:)
...
- (void) refreshRoutedIndicators {
    for (NSRunningApplication* app in appRowViews) {
        ...
        for (NSView* subview in rowView.subviews) {
            if ([subview isKindOfClass:[WCAVM_AppNameLabel class]]) {
...
@implementation WCAVM_AppNameLabel
...
@implementation WCAVM_ShowMoreControlsButton
...
- (void) setUpWithApp:(NSRunningApplication*)app
    ...
    // WCAppVolumes::insertRowForApp:..., after every subview's setUpWithApp: has already run),
...
- (void) bgm_syncHighlightForCurrentControls {
    ...
    for (NSView* view in self.superview.subviews) {
        if ([view isKindOfClass:[WCAVM_EQBandSlider class]]) {
            if (((NSSlider*)view).floatValue != 0.0f) {
                hasNonDefaultControls = YES;
                break;
            }
        } else if ([view isKindOfClass:[WCAVM_PanSlider class]]) {
...
@implementation WCAVM_VolumeMute {
    pid_t appProcessID;
...
- (WCAVM_VolumeSlider* __nullable) findSiblingVolumeSlider {
    for (NSView* view in self.superview.subviews) {
        if ([view isKindOfClass:[WCAVM_VolumeSlider class]]) {
            return (WCAVM_VolumeSlider*)view;
        }
    }
    return nil;
}
...
#pragma unused (ctx, rowView)
    ...
    WCAVM_VolumeSlider* slider = [self findSiblingVolumeSlider];
...
@implementation WCAVM_VolumeSlider {
...
- (void) appVolumeChanged {
    ...
    DebugMsg("WCAppVolumes::appVolumeChanged: App volume for %s (%d) changed to %d",
    ...
    for (NSView* subview in rowView.subviews) {
        if ([subview isKindOfClass:[WCAVM_VolumeMute class]]) {
            [(WCAVM_VolumeMute*)subview bgm_syncForVolume:self.intValue];
        }
    }
}
...
@implementation WCAVM_PanSlider {
    // Will be set to -1 for apps without a pid
    pid_t appProcessID;
    NSString* __nullable appBundleID;
    WCAppVolumesController* controller;
...
- (void) appPanPositionChanged {
    ...
    DebugMsg("WCAppVolumes::appPanPositionChanged: App pan position for %s changed to %d", appBundleID.UTF8String, self.intValue);
...
@implementation WCAVM_EQBandSlider {
    pid_t appProcessID;
    NSString* __nullable appBundleID;
    WCAppVolumesController* controller;
}
...
- (NSArray<WCAVM_EQBandSlider*>*) siblingBandSlidersIncludingSelf {
    NSMutableArray<WCAVM_EQBandSlider*>* sliders = [NSMutableArray new];

    for (NSView* view in self.superview.subviews) {
        if ([view isKindOfClass:[WCAVM_EQBandSlider class]]) {
            [sliders addObject:(WCAVM_EQBandSlider*)view];
        }
    }

    [sliders sortUsingComparator:^NSComparisonResult(WCAVM_EQBandSlider* a, WCAVM_EQBandSlider* b) {
...
- (void) appEQGainChanged {
    NSArray<WCAVM_EQBandSlider*>* sliders = [self siblingBandSlidersIncludingSelf];

    if (sliders.count != kBGMAppEQNumBands) {
        DebugMsg("WCAppVolumes::appEQGainChanged: Expected %d EQ band sliders, found %lu",
                 kBGMAppEQNumBands,
    ...
    for (WCAVM_EQBandSlider* slider in sliders) {
        [gainsDB addObject:@(slider.floatValue)];
    }

    DebugMsg("WCAppVolumes::appEQGainChanged: EQ gains for %s (%d) changed to %s",
             appBundleID.UTF8String,
             appProcessID,
...
@implementation WCAVM_OutputRouteButton {
    NSString* __nullable appBundleID;
    NSString* __nullable appName;
    WCAppOutputRoutingController* routingController;
}
...
- (void) setUpWithApp:(NSRunningApplication*)app
    ...
    // Per-app output routing needs macOS 26+ (WCTapRoute.mm gates on
...
- (void) deviceSelected {
    ...
    // WCAppOutputRoutingController::populateMenuForButton:forBundleID:. Clicking "Default" clears
