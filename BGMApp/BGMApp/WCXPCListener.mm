...
// Local Includes
#import "WCPlayThrough.h"  // For kDeviceNotStarting.


#pragma clang assume_nonnull begin

@implementation WCXPCListener {
...
- (id) initWithAudioDevices:(WCAudioDeviceManager*)devices helperConnectionErrorHandler:(void (^)(NSError* error))errorHandler {
    if ((self = [super init])) {
...
- (void) initHelperConnection {
    ...
    #pragma unused (error)
        ...
        DebugMsg("WCXPCListener::initHelperConnection: Connection error: %s", [[error description] UTF8String]);
    }];
}
...
- (void) initHelperConnectionWithErrorHandler:(void (^)(NSError* error))errorHandler {
    ...
    helperConnection.remoteObjectInterface = [NSXPCInterface interfaceWithProtocol:@protocol(WCXPCHelperXPCProtocol)];
    
    // Export this object so BGMXPCHelper can send messages through helperConnection if it wants to.
    helperConnection.exportedInterface = [NSXPCInterface interfaceWithProtocol:@protocol(WCAppXPCProtocol)];
    ...
    void (^retryAfterTimeout)(NSError*) = ^(NSError* error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"WCXPCListener::initHelperConnectionWithErrorHandler: Lost connection to BGMXPCHelper. Will try to reconnect "
    ...
    helperConnection.interruptionHandler = ^{
        DebugMsg("WCXPCListener::initHelperConnectionWithErrorHandler: Connection to BGMXPCHelper interrupted. Trying to reconnect.");
    ...
    [[helperConnection remoteObjectProxyWithErrorHandler:errorHandler] registerAsBGMAppWithListenerEndpoint:[listener endpoint] reply:^{
        ...
        if (retryTimer) {
            NSLog(@"WCXPCListener::initHelperConnectionWithErrorHandler: Reconnected to BGMXPCHelper.");
        ...
        helperConnection.invalidationHandler = ^{
            DebugMsg("WCXPCListener::initHelperConnectionWithErrorHandler: Connection to BGMXPCHelper invalidated");
            helperConnection = nil;
...
- (BOOL) listener:(NSXPCListener*)listener shouldAcceptNewConnection:(NSXPCConnection*)newConnection {
    ...
    DebugMsg("WCXPCListener::listener: Received new connection");
    
    // Configure the connection.
    // First, set the interface that the exported object implements.
    newConnection.exportedInterface = [NSXPCInterface interfaceWithProtocol:@protocol(WCAppXPCProtocol)];
...
- (void) startPlayThroughSyncWithReply:(void (^)(NSError*))reply forUISoundsDevice:(BOOL)isUI {
    ...
    try {
        ...
        LogError("WCXPCListener::startPlayThroughSyncWithReply: Caught CAException (%d). Replying kBGMXPC_HardwareError.",
                 e.GetError());
        err = kBGMXPC_HardwareError;
    } catch (...) {
        LogError("WCXPCListener::startPlayThroughSyncWithReply: Caught unknown exception. Replying kBGMXPC_InternalError.");
