...
//  WCAutoPauseMusic (which pauses one specific, known music-player app when anything else plays).
...
//
//  Unlike WCAutoPauseMusic, this doesn't send an app-level pause/unpause command -- it works
...
@interface WCDoNotDisturb : NSObject

- (instancetype) initWithAudioDevices:(WCAudioDeviceManager*)audioDevices
                          userDefaults:(WCUserDefaults*)userDefaults;

@property (nonatomic, readonly) BOOL isEnabled;

// Muting/restoring app volumes can throw (the same CoreAudio HAL calls WCHotkeys/WCAppVolumes
...
// a Do Not Disturb mute -- see WCTroubleshootMenu's "Reset All App Volumes, Pan & EQ", which skips
