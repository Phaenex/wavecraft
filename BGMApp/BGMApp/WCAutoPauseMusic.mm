...
@implementation WCAutoPauseMusic {
    BOOL enabled;
...
- (id) initWithAudioDevices:(WCAudioDeviceManager*)inAudioDevices musicPlayers:(WCMusicPlayers*)inMusicPlayers userDefaults:(WCUserDefaults*)inUserDefaults {
    if ((self = [super init])) {
...
- (void) initListenerBlock {
    ...
    __unsafe_unretained WCAutoPauseMusic* weakSelf = self;
    ...
    listenerBlock = ^(UInt32 inNumberAddresses, const AudioObjectPropertyAddress * _Nonnull inAddresses) {
        ...
        for (int i = 0; i < inNumberAddresses; i++) {
            if (inAddresses[i].mSelector == kAudioDeviceCustomPropertyDeviceAudibleState) {
                ...
                DebugMsg("WCAutoPauseMusic::initListenerBlock: kAudioDeviceCustomPropertyDeviceAudibleState property changed to '%s'",
                         audibleStateStr);
#endif
                ...
                if (audibleState == kBGMDeviceIsAudible) {
                    ...
                    DebugMsg("WCAutoPauseMusic: Device is silent except music, resetting wePaused flag");
                    wePaused = NO;
                }
...
- (void) queuePauseBlock {
    ...
    if (pauseDelayMS == 0) {
        DebugMsg("WCAutoPauseMusic::queuePauseBlock: Pause delay is 0, pausing immediately");
    ...
    DebugMsg("WCAutoPauseMusic::queuePauseBlock: Dispatching pause block at %llu", now);
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, pauseDelayNSec),
                   ...
                   ^{
                       ...
                       DebugMsg("WCAutoPauseMusic::queuePauseBlock: Running pause block dispatched at %llu.%s wentAudible=%llu",
                                startedPauseDelay,
...
- (void) queueUnpauseBlock {
    ...
    if (maxUnpauseDelayMS == 0) {
        DebugMsg("WCAutoPauseMusic::queueUnpauseBlock: Max unpause delay is 0, unpausing immediately");
        ...
        DebugMsg("WCAutoPauseMusic::queueUnpauseBlock: Immediate unpause - wePaused=%s, currentState=%s", 
                 wePaused ? "YES" : "NO", 
    ...
    DebugMsg("WCAutoPauseMusic::queueUnpauseBlock: Dispatched unpause block at %llu. unpauseDelayNsec=%llu",
             now,
             unpauseDelayNsec);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, unpauseDelayNsec),
                   ...
                   ^{
                       ...
                       DebugMsg("WCAutoPauseMusic::queueUnpauseBlock: Running unpause block dispatched at %llu. wePaused=%s, isLatest=%s, currentState=%s, wentSilent=%llu",
                                startedUnpauseDelay,
                                wePaused ? "YES" : "NO",
                       ...
                       if (wePaused && isLatestUnpause && stillSilent) {
                           DebugMsg("WCAutoPauseMusic::queueUnpauseBlock: Unpausing music player");
                           wePaused = NO;
                           [musicPlayers.selectedMusicPlayer unpause];
                       }
                   });
