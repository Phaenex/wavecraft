...
//
//  WCAppOutputRoutingController.mm
//  BGMApp
//
...
@implementation WCAppOutputRoutingController {
    ...
    std::map<std::string, std::unique_ptr<WCTapRoute>> routes;
...
- (instancetype) initWithUserDefaults:(WCUserDefaults*)inUserDefaults {
    if ((self = [super init])) {
        ...
        routingQueue = dispatch_queue_create("WCAppOutputRoutingController", DISPATCH_QUEUE_SERIAL);
        ...
        // Watch for a routed device disconnecting mid-route -- without this, WCTapRoute's own
...
- (void) dealloc {
    ...
    // WCTapRoute's real destructor, tearing down its CoreAudio tap/aggregate device -- happens on
...
- (void) listenForDevicesAddedOrRemoved {
    WCAppOutputRoutingController* __weak weakSelf = self;
        ...
        for (NSString* bundleID in assignments) {
            ...
            for (NSString* deviceUID in deviceUIDs) {
                if ([self findConnectedDeviceIDForUID:deviceUID] == kAudioObjectUnknown) {
                    DebugMsg("WCAppOutputRoutingController::removeRoutesForDisconnectedDevices: "
...
- (void) restoreRoutesForApps:(NSArray<NSRunningApplication*>*)apps {
    ...
    for (NSRunningApplication* app in apps) {
        ...
        if (bundleID && uids.count > 0) {
            DebugMsg("WCAppOutputRoutingController::restoreRoutesForApps: "
...
- (void) observeValueForKeyPath:(NSString* __nullable)keyPath
    ...
    if (keyPath && change && [keyPath isEqualToString:@"runningApplications"]) {
        ...
        // WCTapRoute's processRestoreEnabled: a route is meant to keep waiting for its app to
...
- (void) applyRouteForBundleID:(NSString*)bundleID
    ...
    if (deviceUIDs.count == 0) {
        // Back to Default -- tear down the whole route. WCTapRoute::~WCTapRoute calls Stop(),
    ...
    for (NSString* uid in deviceUIDs) {
        ...
        if (deviceID == kAudioObjectUnknown) {
            DebugMsg("WCAppOutputRoutingController::applyRouteForBundleID: "
    ...
    WCTapRoute* __nullable route = nullptr;

    if (it == routes.end()) {
        // No existing route for this app -- create the tap fresh, with no outputs yet.
        try {
            // WCTapRoute retains this for its lifetime (see its header), so give it a CACFString
            ...
            auto newRoute = std::unique_ptr<WCTapRoute>(new WCTapRoute(ownedBundleID));
            newRoute->Start();
            route = newRoute.get();
            ...
            LogError("WCAppOutputRoutingController::applyRouteForBundleID: "
    ...
    // in WCOutputDeviceDiffTests.mm) deliberately leaves an output alone if it's already correct,
    // so e.g. switching the target set from {A, B} to {A, C} doesn't glitch A's audio by tearing
    // its WCPlayThrough down and immediately back up for no reason.
    ...
    WCOutputDeviceDiff diff = BGMComputeOutputDeviceDiff(currentDeviceIDs, desiredDeviceIDs);
...
// though its WCTapRoute output never actually started, with no way to notice short of "Remove
...
- (void) reconcilePersistedAssignmentForBundleID:(NSString*)bundleID key:(const std::string&)key {
    ...
    WCAppOutputRoutingController* __weak weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        WCAppOutputRoutingController* __strong strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
// WCTapRoute::Start()/AddOutputDevice() distinguish these two specific causes from a generic
// CoreAudio failure -- see WCTapRoute.h. Only the fallback case is actually about a raw OSStatus;
...
- (NSString*) reasonForTapRouteException:(const CAException&)e {
    LogError("WCAppOutputRoutingController: CAException %d", static_cast<int>(e.GetError()));

    if (e.GetError() == WCTapRoute::kMacOSTooOld) {
        return @"Per-app output routing needs macOS 26 or later.";
    } else if (e.GetError() == WCTapRoute::kOutputDeviceVanished) {
