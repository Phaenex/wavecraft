...
@class WCOutputVolumeMenuItem;
@class WCOutputDeviceMenuSection;
...
@interface WCAudioDeviceManager : NSObject

// Returns nil if BGMDevice isn't installed.
- (instancetype) init;

// Set the WCOutputVolumeMenuItem to be notified when the output device is changed.
- (void) setOutputVolumeMenuItem:(WCOutputVolumeMenuItem*)item;

// Set the WCOutputDeviceMenuSection to be notified when the output device is changed.
...
// is running. See WCPlayThrough.
//
// Returns one of the error codes defined by this class or WCPlayThrough, or an AudioHardware error
// code received from the HAL.
- (OSStatus) startPlayThroughSync:(BOOL)forUISoundsDevice;

// When the output device is changed, WCAudioDeviceManager will send the ID of the new output
