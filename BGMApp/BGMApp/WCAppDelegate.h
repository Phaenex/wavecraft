...
// Loaded from the XIB but never shown directly -- WCMainPanel replaced it as the main dropdown
// (see that class's header for why). Its only remaining job is supplying the Preferences submenu's
// static structure to WCPreferencesMenu, which still uses a real NSMenu popped up on demand (see
// WCPreferencesMenu.menu) since none of its content has the interaction risk that motivated
...
@property (readonly) WCAudioDeviceManager* audioDevices;
@property WCAppVolumesController* appVolumes;
// Exposed publicly (mirroring appVolumes) so AppleScript support can reach it -- see
// WCAppDelegate+AppleScript.mm.
@property (readonly) WCAppOutputRoutingController* outputRoutingController;
