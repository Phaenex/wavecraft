...
//
//  WCPlayThroughRTLogger.h
//  BGMApp
//
//  Copyright © 2020 Kyle Neideck
//
//  A real-time safe logger for WCPlayThrough. The messages are logged asynchronously by a
//  non-realtime thread.
//
//  For the sake of simplicity, this class is very closely coupled with WCPlayThrough and its
...
class WCPlayThroughRTLogger
{

#pragma mark Construction/Destruction

public:
                            WCPlayThroughRTLogger();
                            ~WCPlayThroughRTLogger();
                            WCPlayThroughRTLogger(const WCPlayThroughRTLogger&) = delete;
                            WCPlayThroughRTLogger& operator=(
                                    const WCPlayThroughRTLogger&) = delete;
private:
...
public:
    /*! For WCPlayThrough::ReleaseThreadsWaitingForOutputToStart. */
    void                    LogReleasingWaitingThreads();
    /*! For WCPlayThrough::ReleaseThreadsWaitingForOutputToStart. */
    void                    LogIfMachError_ReleaseWaitingThreadsSignal(mach_error_t inError);

    /*! For WCPlayThrough::OutputDeviceIOProc. Not thread-safe. */
    ...
    /*! For WCPlayThrough::UpdateIOProcState. Not thread-safe. */
...
private:
    ...
    static void* __nullable LoggingThreadEntry(WCPlayThroughRTLogger* inRefCon);
...
private:
    ...
    // For WCPlayThrough::UpdateIOProcState
    struct {
        const char* callerName;
        int ioState;
        std::atomic<bool> shouldLogMessage { false };
