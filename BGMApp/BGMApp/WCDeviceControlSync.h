...
//  the default device.
//
//  WCDeviceControlSync disables any BGMDevice controls that the output device doesn't also have.
//  When the value of one of BGMDevice's controls is changed, WCDeviceControlSync copies the new
...
class WCDeviceControlSync
{

#pragma mark Construction/Destruction

public:
                        WCDeviceControlSync(AudioObjectID inBGMDevice,
                                             AudioObjectID inOutputDevice,
                                             CAHALAudioSystemObject inAudioSystem
                                                     = CAHALAudioSystemObject());
                        ~WCDeviceControlSync();
                        // Disallow copying
                        WCDeviceControlSync(const WCDeviceControlSync&) = delete;
                        WCDeviceControlSync& operator=(const WCDeviceControlSync&) = delete;

#ifdef __OBJC__
                        // Only intended as a convenience for Objective-C instance vars
                        WCDeviceControlSync()
                        : WCDeviceControlSync(kAudioObjectUnknown, kAudioObjectUnknown) { };
#endif

    /*!
     Begin synchronising BGMDevice's controls with the output device's.

     @throws WC_DeviceNotSetException if BGMDevice isn't set.
     @throws CAException if the HAL or one of the devices returns an error when this function
                         ...
                         WCDeviceControlSync will remain inactive if this function throws.
     */
    void                Activate();
...
#pragma mark Accessors
    ...
    /*!
     ...
     @throws CAException if the HAL or one of the new devices returns an error while restarting
                         synchronisation. This WCDeviceControlSync will be deactivated if this
...
private:
    ...
    WCDeviceControlsList mBGMDeviceControlsList;
