...
@implementation WCOutputDeviceMenuSection {
    NSStackView* deviceStack;
    WCAudioDeviceManager* audioDevices;
    WCPreferredOutputDevices* preferredDevices;
...
- (void) listenForDevicesAddedOrRemoved {
    ...
    WCOutputDeviceMenuSection* __weak weakSelf = self;
...
- (void) populateDeviceList {
    ...
    BGMAssert([NSThread isMainThread],
              "WCOutputDeviceMenuSection::populateDeviceList called on non-main thread");

    // Remove existing buttons.
    for (NSButton* button in outputDeviceButtons) {
        DebugMsg("WCOutputDeviceMenuSection::populateDeviceList: Removing %s",
                 button.description.UTF8String);
        // The button's container row (see WCMainPanelContentView's rowContainerWithControl:
...
- (void) insertButtonsForDevice:(BGMAudioDevice)device {
    ...
    if (canBeOutputDevice) {
        for (NSButton* button : [self createButtonsForDevice:device]) {
            DebugMsg("WCOutputDeviceMenuSection::insertButtonsForDevice: Inserting %s",
                     button.description.UTF8String);
...
- (NSArray<NSButton*>*) createButtonsForDevice:(CAHALAudioDevice)device {
    ...
    if (numDataSources > 0) {
        ...
        for (UInt32 i = 0; i < numDataSources; i++) {
            DebugMsg("WCOutputDeviceMenuSection::createButtonsForDevice: "
        ...
        DebugMsg("WCOutputDeviceMenuSection::createButtonsForDevice: Creating button. %s%u",
                 "Device ID:", device.GetObjectID());
...
// WCMainPanelContentView.rowContainerWithControl:height:), which is what actually gets added to
// deviceStack.
- (NSButton*) createButtonForDevice:(CAHALAudioDevice)device
    ...
    NSView* row = [WCMainPanelContentView rowContainerWithControl:button height:kBGMMainPanelRowHeight];
    [deviceStack addArrangedSubview:row];

    return button;
}

// Called by WCAudioDeviceManager to tell us a different device has been set as the output device.
...
- (void) outputDeviceButtonClicked:(NSButton*)button {
    DebugMsg("WCOutputDeviceMenuSection::outputDeviceButtonClicked: '%s' button clicked",
