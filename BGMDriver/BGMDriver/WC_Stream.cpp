...
WC_Stream::WC_Stream(AudioObjectID inObjectID,
                       AudioDeviceID inOwnerDeviceID,
                       bool inIsInput,
                       Float64 inSampleRate,
                       UInt32 inStartingChannel)
:
    WC_Object(inObjectID, kAudioStreamClassID, kAudioObjectClassID, inOwnerDeviceID),
...
bool    WC_Stream::HasProperty(AudioObjectID inObjectID,
                                pid_t inClientPID,
...
bool    WC_Stream::IsPropertySettable(AudioObjectID inObjectID,
                                       pid_t inClientPID,
                                       const AudioObjectPropertyAddress& inAddress) const
{
    // For each object, this driver implements all the required properties plus a few extras that
...
UInt32    WC_Stream::GetPropertyDataSize(AudioObjectID inObjectID,
                                          pid_t inClientPID,
                                          ...
                                          const void* __nullable inQualifierData) const
{
    ...
    switch(inAddress.mSelector)
    {
        ...
        default:
            theAnswer = WC_Object::GetPropertyDataSize(inObjectID,
                                                        inClientPID,
                                                        inAddress,
                                                        inQualifierDataSize,
                                                        inQualifierData);
...
void    WC_Stream::GetPropertyData(AudioObjectID inObjectID,
                                    pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyBaseClass:
            ...
            ThrowIf(inDataSize < sizeof(AudioClassID),
                    ...
                    "WC_Stream::GetPropertyData: not enough space for the return "
        ...
        case kAudioStreamPropertyDirection:
            ...
            ThrowIf(inDataSize < sizeof(UInt32),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "WC_Stream::GetPropertyData: not enough space for the return value of "
        ...
        default:
            WC_Object::GetPropertyData(inObjectID,
                                        inClientPID,
                                        inAddress,
                                        inQualifierDataSize,
                                        inQualifierData,
...
void    WC_Stream::SetPropertyData(AudioObjectID inObjectID,
                                    pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
                                    const void* __nullable inQualifierData,
        ...
        case kAudioStreamPropertyIsActive:
            {
                ...
                ThrowIf(inDataSize != sizeof(UInt32),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "WC_Stream::SetPropertyData: wrong size for the data for "
                ...
                if(mIsStreamActive != theNewIsActive)
                {
                    ...
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,	^{
                        ...
                        WC_PlugIn::Host_PropertiesChanged(inObjectID, 1, theProperty);
                    });
                }
            }
            break;
        ...
        case kAudioStreamPropertyPhysicalFormat:
            {
                ...
                ThrowIf(theNewFormat->mFormatID != kAudioFormatLinearPCM,
                        ...
                        "WC_Stream::SetPropertyData: unsupported format ID for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mFormatFlags !=
                        ...
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported format flags for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBytesPerPacket != 8,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported bytes per packet for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mFramesPerPacket != 1,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported frames per packet for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBytesPerFrame != 8,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported bytes per frame for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mChannelsPerFrame != 2,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported channels per frame for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBitsPerChannel != 32,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported bits per channel for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mSampleRate < 1.0,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "WC_Stream::SetPropertyData: unsupported sample rate for "
                        "kAudioStreamPropertyPhysicalFormat");
            }
            break;

        default:
            WC_Object::SetPropertyData(inObjectID,
                                        inClientPID,
                                        inAddress,
                                        inQualifierDataSize,
                                        inQualifierData,
...
void    WC_Stream::SetSampleRate(Float64 inSampleRate)
{
    CAMutex::Locker theStateLocker(mStateMutex);
    mSampleRate = inSampleRate;
}
