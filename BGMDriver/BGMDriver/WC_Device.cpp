...
pthread_once_t				WC_Device::sStaticInitializer = PTHREAD_ONCE_INIT;
WC_Device*					WC_Device::sInstance = nullptr;
WC_Device*					WC_Device::sUISoundsInstance = nullptr;

WC_Device&	WC_Device::GetInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sInstance;
}

WC_Device&	WC_Device::GetUISoundsInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sUISoundsInstance;
}

void	WC_Device::StaticInitializer()
{
    try
    {
        ...
        sInstance = new WC_Device(kObjectID_Device,
                                   CFSTR(kDeviceName),
								   CFSTR(kBGMDeviceUID),
        ...
        sUISoundsInstance = new WC_Device(kObjectID_Device_UI_Sounds,
        ...
        WC_VolumeControl& theUISoundsVolumeControl = sUISoundsInstance->mVolumeControl;
        ...
        // Apply the volume to the device's output stream. The main instance of WC_Device doesn't
    ...
    {
        DebugMsg("WC_Device::StaticInitializer: failed to create the devices");
...
WC_Device::WC_Device(AudioObjectID inObjectID,
...
:
	WC_AbstractDevice(inObjectID, kAudioObjectPlugInObject),
...
{
    // mSampleRateForRT (see WC_Device.h) has to be genuinely lock-free -- ApplyClientEQ reads it
    ...
    ThrowIf(!mSampleRateForRT.is_lock_free(),
            ...
            "WC_Device::WC_Device: std::atomic<Float64> is not lock-free on this platform");
...
void	WC_Device::Activate()
{
	...
	WC_AbstractDevice::Activate();
}

void	WC_Device::Deactivate()
{
	...
	WC_AbstractDevice::Deactivate();
	
}

void    WC_Device::InitLoopback()
{
...
bool	WC_Device::HasProperty(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress) const
{
...
bool	WC_Device::IsPropertySettable(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress) const
{
	bool theAnswer = false;
...
UInt32	WC_Device::GetPropertyDataSize(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData) const
{
	UInt32 theAnswer = 0;
...
void	WC_Device::GetPropertyData(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32& outDataSize, void* outData) const
{
    ThrowIfNULL(outData, std::runtime_error("!outData"), "WC_Device::GetPropertyData: !outData");
...
void	WC_Device::SetPropertyData(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData)
{
    ThrowIfNULL(inData, std::runtime_error("no data"), "WC_Device::SetPropertyData: no data");
...
bool	WC_Device::Device_HasProperty(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress) const
{
	...
	switch(inAddress.mSelector)
	{
		...
		default:
			theAnswer = WC_AbstractDevice::HasProperty(inObjectID, inClientPID, inAddress);
			break;
	};
	return theAnswer;
}

bool	WC_Device::Device_IsPropertySettable(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress) const
{
	//	For each object, this driver implements all the required properties plus a few extras that
	//	are useful but not required. There is more detailed commentary about each property in the
	//	Device_GetPropertyData() method.
	...
	switch(inAddress.mSelector)
    {
		...
		default:
			theAnswer = WC_AbstractDevice::IsPropertySettable(inObjectID, inClientPID, inAddress);
			break;
	};
	return theAnswer;
}

UInt32	WC_Device::Device_GetPropertyDataSize(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData) const
{
	//	For each object, this driver implements all the required properties plus a few extras that
	//	are useful but not required. There is more detailed commentary about each property in the
	//	Device_GetPropertyData() method.
	...
	switch(inAddress.mSelector)
	{
        ...
        case kAudioObjectPropertyCustomPropertyInfoList:
            // kNumCustomProperties (WC_Device.h) is also used by Device_GetPropertyData's clamp
		...
		default:
			theAnswer = WC_AbstractDevice::GetPropertyDataSize(inObjectID, inClientPID, inAddress, inQualifierDataSize, inQualifierData);
			break;
	};
...
void	WC_Device::Device_GetPropertyData(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32& outDataSize, void* outData) const
{
	//	For each object, this driver implements all the required properties plus a few extras that
	...
	switch(inAddress.mSelector)
	{
		case kAudioObjectPropertyName:
			...
			ThrowIf(inDataSize < sizeof(AudioObjectID), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioObjectPropertyName for the device");
		...
		case kAudioObjectPropertyManufacturer:
			...
			ThrowIf(inDataSize < sizeof(AudioObjectID), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioObjectPropertyManufacturer for the device");
		...
		case kAudioDevicePropertyDeviceUID:
			...
			ThrowIf(inDataSize < sizeof(AudioObjectID), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyDeviceUID for the device");
		...
		case kAudioDevicePropertyModelUID:
			...
			ThrowIf(inDataSize < sizeof(AudioObjectID), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyModelUID for the device");
		...
		case kAudioDevicePropertyDeviceIsRunning:
            ...
            ThrowIf(inDataSize < sizeof(UInt32), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyDeviceIsRunning for the device");
            *reinterpret_cast<UInt32*>(outData) = mClients.IsIORunning() ? 1 : 0;
            outDataSize = sizeof(UInt32);
			break;

        case kAudioDevicePropertyDeviceCanBeDefaultDevice:
            // See WC_AbstractDevice::GetPropertyData.
			//
			// We don't allow the UI Sounds instance of WC_Device to be set as the default device
            ...
            ThrowIf(inDataSize < sizeof(UInt32),
                    ...
                    "WC_Device::GetPropertyData: not enough space for the return value of "
                    "kAudioDevicePropertyDeviceCanBeDefaultDevice for the device");
            // TODO: Add a field for this and set it in WC_Device::StaticInitializer so we don't
		...
		case kAudioDevicePropertyNominalSampleRate:
            ...
            ThrowIf(inDataSize < sizeof(Float64),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyNominalSampleRate for the device");
		...
		case kAudioDevicePropertyAvailableNominalSampleRates:
			...
			if(theNumberItemsToFetch > 0)
			{
                // 0 would cause divide-by-zero errors in other WC_Device functions (and
		...
		case kAudioDevicePropertyPreferredChannelsForStereo:
			...
			ThrowIf(inDataSize < (2 * sizeof(UInt32)), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyPreferredChannelsForStereo for the device");
			((UInt32*)outData)[0] = 1;
			((UInt32*)outData)[1] = 2;
		...
		case kAudioDevicePropertyPreferredChannelLayout:
			...
			//	by default. For this device, we return a stereo ACL.
			{
				...
				ThrowIf(inDataSize < theACLSize, CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyPreferredChannelLayout for the device");
		...
		case kAudioDevicePropertyZeroTimeStampPeriod:
			...
			ThrowIf(inDataSize < sizeof(UInt32), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyZeroTimeStampPeriod for the device");
        ...
        case kAudioDevicePropertyIcon:
            {
                ...
                ThrowIf(inDataSize < sizeof(CFURLRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDevicePropertyIcon for the device");
                
                CFBundleRef theBundle = CFBundleGetBundleWithIdentifier(WC_PlugIn::GetInstance().GetBundleID());
                ThrowIf(theBundle == NULL, CAException(kAudioHardwareUnspecifiedError), "WC_Device::Device_GetPropertyData: could not get the plugin bundle for kAudioDevicePropertyIcon");
                
                CFURLRef theURL = CFBundleCopyResourceURL(theBundle, CFSTR("DeviceIcon.icns"), NULL, NULL);
                ThrowIf(theURL == NULL, CAException(kAudioHardwareUnspecifiedError), "WC_Device::Device_GetPropertyData: could not get the URL for kAudioDevicePropertyIcon");
        ...
        case kAudioDeviceCustomPropertyDeviceAudibleState:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyDeviceAudibleState for the device");
        ...
        case kAudioDeviceCustomPropertyMusicPlayerProcessID:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyMusicPlayerProcessID for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyMusicPlayerBundleID:
            {
                ThrowIf(inDataSize < sizeof(CFStringRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyMusicPlayerBundleID for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanBGMApp:
            ThrowIf(inDataSize < sizeof(CFBooleanRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanBGMApp for the device");
        ...
        case kAudioDeviceCustomPropertyDebugLoggingEnabled:
            ThrowIf(inDataSize < sizeof(CFBooleanRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyDebugLoggingEnabled for the device");
        ...
        case kAudioDeviceCustomPropertyAppVolumes:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyAppVolumes for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyAppEQ:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyAppEQ for the device");
                CAMutex::Locker theStateLocker(mStateMutex);
        ...
        case kAudioDeviceCustomPropertyEnabledOutputControls:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_GetPropertyData: not enough space for the return value of kAudioDeviceCustomPropertyEnabledOutputControls for the device");
		...
		default:
			WC_AbstractDevice::GetPropertyData(inObjectID, inClientPID, inAddress, inQualifierDataSize, inQualifierData, inDataSize, outDataSize, outData);
			break;
	};
}
...
static void ValidateAppVolumesProperty(const CACFArray& inAppVolumes)
{
    ...
    for(UInt32 i = 0; i < theCount; i++)
    {
        ...
        ThrowIf(!didGetValue || theElement == nullptr,
                ...
                "WC_Device::ValidateAppVolumesProperty: Could not get element from array");
        ThrowIf(CFGetTypeID(theElement) != CFDictionaryGetTypeID(),
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppVolumesProperty: Element is not a CFDictionary");
        ...
        if(hasPID && thePIDValue != nullptr)
        {
            ThrowIf(CFGetTypeID(thePIDValue) != CFNumberGetTypeID(),
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppVolumesProperty: ProcessID is not a CFNumber");
        }
        ...
        if(hasBundleID && theBundleIDValue != nullptr)
        {
            ThrowIf(CFGetTypeID(theBundleIDValue) != CFStringGetTypeID(),
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppVolumesProperty: BundleID is not a CFString");
        }

        // At least one of ProcessID or BundleID must be present.
        ThrowIf(!hasPID && !hasBundleID,
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppVolumesProperty: Neither ProcessID nor BundleID present");
        ...
        if(hasVolume)
        {
            ThrowIf(theVolume < kAppRelativeVolumeMinRawValue ||
                    ...
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppVolumesProperty: RelativeVolume out of range");
        }
        ...
        if(hasPan)
        {
            ThrowIf(thePan < kAppPanLeftRawValue || thePan > kAppPanRightRawValue,
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppVolumesProperty: PanPosition out of range");
        }
    }
}

// kBGMAppEQNumBands (BGM_Types.h, the property-marshaling contract) and WC_AppEQ::kNumBands
// (WC_Biquad.h, the real-time DSP band count) are independent constants with no other mechanism
...
// for what WC_Clients::SetClientsEQ actually iterates. Catch that at compile time instead.
static_assert(kBGMAppEQNumBands == WC_AppEQ::kNumBands,
              "kBGMAppEQNumBands (BGM_Types.h) and WC_AppEQ::kNumBands (WC_Biquad.h) must match "
...
static void ValidateAppEQProperty(const CACFArray& inAppEQ)
{
    ...
    for(UInt32 i = 0; i < theCount; i++)
    {
        ...
        ThrowIf(!didGetValue || theElement == nullptr,
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: Could not get element from array");
        ThrowIf(CFGetTypeID(theElement) != CFDictionaryGetTypeID(),
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: Element is not a CFDictionary");
        ...
        if(hasPID && thePIDValue != nullptr)
        {
            ThrowIf(CFGetTypeID(thePIDValue) != CFNumberGetTypeID(),
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppEQProperty: ProcessID is not a CFNumber");
        }
        ...
        if(hasBundleID && theBundleIDValue != nullptr)
        {
            ThrowIf(CFGetTypeID(theBundleIDValue) != CFStringGetTypeID(),
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppEQProperty: BundleID is not a CFString");
        }

        // At least one of ProcessID or BundleID must be present.
        ThrowIf(!hasPID && !hasBundleID,
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: Neither ProcessID nor BundleID present");
        ...
        ThrowIf(!hasBandGains || theBandGainsValue == nullptr,
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: BandGains not present");
        ThrowIf(CFGetTypeID(theBandGainsValue) != CFArrayGetTypeID(),
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: BandGains is not a CFArray");
        ...
        ThrowIf(theBandGains.GetNumberItems() != kBGMAppEQNumBands,
                CAException(kAudioHardwareIllegalOperationError),
                "WC_Device::ValidateAppEQProperty: BandGains must have exactly kBGMAppEQNumBands items");
        ...
        for(UInt32 band = 0; band < kBGMAppEQNumBands; band++)
        {
            ...
            ThrowIf(!didGetBand || theBandValue == nullptr,
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppEQProperty: Could not get band gain from array");
            ThrowIf(CFGetTypeID(theBandValue) != CFNumberGetTypeID(),
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppEQProperty: Band gain is not a CFNumber");

            Float64 theGain = 0.0;
            theBandGains.GetFloat64(band, theGain);
            ThrowIf(theGain < kBGMAppEQMinGainDB || theGain > kBGMAppEQMaxGainDB,
                    CAException(kAudioHardwareIllegalOperationError),
                    "WC_Device::ValidateAppEQProperty: Band gain out of range");
        }
    }
}

void	WC_Device::Device_SetPropertyData(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData)
{
	switch(inAddress.mSelector)
	{
        case kAudioDevicePropertyNominalSampleRate:
            ThrowIf(inDataSize < sizeof(Float64),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "WC_Device::Device_SetPropertyData: wrong size for the data for kAudioDevicePropertyNominalSampleRate");
            RequestSampleRate(*reinterpret_cast<const Float64*>(inData));
            break;
            
        case kAudioDeviceCustomPropertyMusicPlayerProcessID:
            {
                ThrowIf(inDataSize < sizeof(CFNumberRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                
                CFNumberRef pidRef = *reinterpret_cast<const CFNumberRef*>(inData);
                
                ThrowIf(pidRef == NULL, CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: null reference given for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                ThrowIf(CFGetTypeID(pidRef) != CFNumberGetTypeID(), CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyMusicPlayerProcessID was not a CFNumber");
                ...
                ThrowIf(!success, CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: probable error from CFNumberGetValue when reading pid for kAudioDeviceCustomPropertyMusicPlayerProcessID");
                ...
                catch(WC_InvalidClientPIDException)
                {
                ...
                if(propertyWasChanged)
                {
                    ...
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,	^{
                        ...
                        WC_PlugIn::Host_PropertiesChanged(inObjectID, 2, theChangedProperties);
                    });
                }
            }
            break;
        ...
        case kAudioDeviceCustomPropertyMusicPlayerBundleID:
            {
                ThrowIf(inDataSize < sizeof(CFStringRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyMusicPlayerBundleID");
            
                CFStringRef theBundleIDRef = *reinterpret_cast<const CFStringRef*>(inData);
                
                ThrowIfNULL(theBundleIDRef, CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: kAudioDeviceCustomPropertyMusicPlayerBundleID cannot be set to NULL");
                ThrowIf(CFGetTypeID(theBundleIDRef) != CFStringGetTypeID(), CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyMusicPlayerBundleID was not a CFString");
        ...
        case kAudioDeviceCustomPropertyAppVolumes:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyAppVolumes");
                
                CFArrayRef arrayRef = *reinterpret_cast<const CFArrayRef*>(inData);

                ThrowIfNULL(arrayRef, CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: kAudioDeviceCustomPropertyAppVolumes cannot be set to NULL");
                ThrowIf(CFGetTypeID(arrayRef) != CFArrayGetTypeID(), CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyAppVolumes was not a CFArray");
                ...
                catch(WC_InvalidClientRelativeVolumeException)
                {
                    Throw(CAException(kAudioHardwareIllegalOperationError));
                }
                ...
                if(propertyWasChanged)
                {
                    ...
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,	^{
                        ...
                        WC_PlugIn::Host_PropertiesChanged(inObjectID, 1, theChangedProperties);
                    });
                }
            }
            break;
        ...
        case kAudioDeviceCustomPropertyAppEQ:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef), CAException(kAudioHardwareBadPropertySizeError), "WC_Device::Device_SetPropertyData: wrong size for the data for kAudioDeviceCustomPropertyAppEQ");

                CFArrayRef arrayRef = *reinterpret_cast<const CFArrayRef*>(inData);

                ThrowIfNULL(arrayRef, CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: kAudioDeviceCustomPropertyAppEQ cannot be set to NULL");
                ThrowIf(CFGetTypeID(arrayRef) != CFArrayGetTypeID(), CAException(kAudioHardwareIllegalOperationError), "WC_Device::Device_SetPropertyData: CFType given for kAudioDeviceCustomPropertyAppEQ was not a CFArray");
        ...
        case kAudioDeviceCustomPropertyEnabledOutputControls:
            {
                ThrowIf(inDataSize < sizeof(CFArrayRef),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "WC_Device::Device_SetPropertyData: wrong size for the data for "
                ...
                ThrowIfNULL(theEnabledControlsRef,
                            CAException(kAudioHardwareIllegalOperationError),
                            "WC_Device::Device_SetPropertyData: null reference given for "
                            "kAudioDeviceCustomPropertyEnabledOutputControls");
                ThrowIf(CFGetTypeID(theEnabledControlsRef) != CFArrayGetTypeID(),
                        CAException(kAudioHardwareIllegalOperationError),
                        "WC_Device::Device_SetPropertyData: CFType given for "
                ...
                ThrowIf(theEnabledControls.GetNumberItems() != 2,
                        CAException(kAudioHardwareIllegalOperationError),
                        "WC_Device::Device_SetPropertyData: Expected the CFArray given for "
                ...
                ThrowIf(!didGetBool,
                        CAException(kAudioHardwareIllegalOperationError),
                        "WC_Device::Device_SetPropertyData: Expected CFBoolean for volume elem of "
                        "kAudioDeviceCustomPropertyEnabledOutputControls");
                ...
                ThrowIf(!didGetBool,
                        CAException(kAudioHardwareIllegalOperationError),
                        "WC_Device::Device_SetPropertyData: Expected CFBoolean for mute elem of "
                        "kAudioDeviceCustomPropertyEnabledOutputControls");
        ...
        case kAudioDeviceCustomPropertyDebugLoggingEnabled:
            {
                ...
                DebugMsg("WC_Device::Device_SetPropertyData: Debug logging %s",
		...
		default:
			WC_AbstractDevice::SetPropertyData(inObjectID, inClientPID, inAddress, inQualifierDataSize, inQualifierData, inDataSize, inData);
			break;
    };
}
...
void	WC_Device::StartIO(UInt32 inClientID)
{
    ...
    
    {
        ...
        if(isFirstClient)
        {
            ...
            BGMAssert(mIOMutex.IsFree(),
                      "WC_Device::StartIO: IO mutex taken before starting IO");
            mAudibleState.Reset();
        }
    ...
    if(!clientIsBGMApp && bgmAppHasClientRegistered)
    {
        ...
        switch(theXPCError)
        {
            ...
            case kBGMXPC_MessageFailure:
                ...
                LogWarning("WC_Device::StartIO: Couldn't reach BGMApp via XPC. Attempting to start IO anyway.");
                break;
                       
           case kBGMXPC_Timeout:
               // XPC timeout. IO will probably still work,
               // but we may drop frames while the audio hardware starts up.
               LogWarning("WC_Device::StartIO: Couldn't reach BGMApp via XPC (timeout). Attempting to start IO anyway.");
               break;
            ...
            case kBGMXPC_ReturningEarlyError:
                ...
                // See WCAudioDeviceManager::startPlayThroughSync and WCPlayThrough::WaitForOutputDeviceToStart.
                LogWarning("WC_Device::StartIO: BGMApp was busy, so BGMDriver has to return from StartIO early.");
                break;
                
            default:
                DebugMsg("WC_Device::StartIO: BGMApp failed to start the output device. theXPCError=%llu", theXPCError);
                LogError("WC_Device::StartIO: BGMApp failed to start the output device. theXPCError=%llu", theXPCError);
                //Throw(CAException(kAudioHardwareNotRunningError));
        }
    }
}

void	WC_Device::StopIO(UInt32 inClientID)
{
    #pragma unused(inClientID)
    DebugMsg("WC_Device::StopIO: inClientID=%u", inClientID);
    mClients.StopIO();
}

void	WC_Device::GetZeroTimeStamp(Float64& outSampleTime, UInt64& outHostTime, UInt64& outSeed)
{
...
void	WC_Device::WillDoIOOperation(UInt32 inOperationID, bool& outWillDo, bool& outWillDoInPlace) const
{
...
void	WC_Device::BeginIOOperation(UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo& inIOCycleInfo, UInt32 inClientID)
{
	...
	//
	// See WC_Clients::mInactivityTimer and kAudioServerPlugInIOOperationProcessOutput for the workaround.
}

void	WC_Device::DoIOOperation(AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo& inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	...
	switch(inOperationID)
	{
        ...
        case kAudioServerPlugInIOOperationProcessMix:
            {
                ...
                ThrowIfNULL(ioMainBuffer,
                            CAException(kAudioHardwareIllegalOperationError),
                            "WC_Device::DoIOOperation: Buffer for "
		...
		default:
			...
			DebugMsg("WC_Device::DoIOOperation: Unexpected IO operation: %u", inOperationID);
			break;
	};
}

void	WC_Device::EndIOOperation(UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo& inIOCycleInfo, UInt32 inClientID)
{
    #pragma unused(inOperationID, inIOBufferFrameSize, inIOCycleInfo, inClientID)
}

void	WC_Device::ReadInputData(UInt32 inIOBufferFrameSize, Float64 inSampleTime, void* outBuffer)
{
...
void	WC_Device::WriteOutputData(UInt32 inIOBufferFrameSize, Float64 inSampleTime, const void* inBuffer)
{
    // Wrap the provided buffer in an AudioBufferList.
    AudioBufferList abl = {};
    abl.mNumberBuffers = 1;
...
void	WC_Device::ApplyClientRelativeVolume(UInt32 inClientID, UInt32 inIOBufferFrameSize, void* ioBuffer) const
{
...
void	WC_Device::ApplyClientEQ(UInt32 inClientID, UInt32 inIOBufferFrameSize, void* ioBuffer)
{
    ...
    // adds/removes clients. See WC_Device.h and docs/LESSONS.md.
    ...
    std::array<Float32, WC_AppEQ::kNumBands> theGainsDB = mClients.GetClientEQBandGainsRT(inClientID);
...
void    WC_Device::RequestEnabledControls(bool inVolumeEnabled, bool inMuteEnabled)
{
    CAMutex::Locker theStateLocker(mStateMutex);
    ...
    if(changeVolume)
    {
        DebugMsg("WC_Device::RequestEnabledControls: %s volume control",
    ...
    if(changeMute)
    {
        DebugMsg("WC_Device::RequestEnabledControls: %s mute control",
    ...
    if(changeVolume || changeMute)
    {
        ...
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,	^{
            WC_PlugIn::Host_RequestDeviceConfigurationChange(theDeviceObjectID, action, nullptr);
        });
    }
}

Float64	WC_Device::GetSampleRate() const
{
...
Float64	WC_Device::GetSampleRateRT() const noexcept
{
...
void	WC_Device::RequestSampleRate(Float64 inRequestedSampleRate)
{
    ...
    ThrowIf(inRequestedSampleRate < 1.0,
            ...
            "WC_Device::RequestSampleRate: unsupported sample rate");

    DebugMsg("WC_Device::RequestSampleRate: Sample rate change requested: %f",
    ...
    if(inRequestedSampleRate != GetSampleRate())  // Check the sample rate will actually be changed.
    {
        ...
        auto requestSampleRate = ^{
            ...
            WC_PlugIn::Host_RequestDeviceConfigurationChange(GetObjectID(), action, nullptr);
        };
...
WC_Object&  WC_Device::GetOwnedObjectByID(AudioObjectID inObjectID)
{
	// C++ is weird. See "Avoid Duplication in const and Non-const Member Functions" in Item 3 of Effective C++.
	return const_cast<WC_Object&>(static_cast<const WC_Device&>(*this).GetOwnedObjectByID(inObjectID));
}

const WC_Object&  WC_Device::GetOwnedObjectByID(AudioObjectID inObjectID) const
{
	...
	{
		LogError("WC_Device::GetOwnedObjectByID: Unknown object ID. inObjectID = %u", inObjectID);
		Throw(CAException(kAudioHardwareBadObjectError));
	}
}

UInt32	WC_Device::GetNumberOfSubObjects() const
{
	return kNumberOfInputSubObjects + GetNumberOfOutputSubObjects();
}

UInt32	WC_Device::GetNumberOfOutputSubObjects() const
{
	return kNumberOfOutputStreams + GetNumberOfOutputControls();
}

UInt32	WC_Device::GetNumberOfOutputControls() const
{
	CAMutex::Locker theStateLocker(mStateMutex);
...
void    WC_Device::SetEnabledControls(bool inVolumeEnabled, bool inMuteEnabled)
{
    CAMutex::Locker theStateLocker(mStateMutex);

    if(mVolumeControl.IsActive() != inVolumeEnabled)
    {
        DebugMsg("WC_Device::SetEnabledControls: %s the volume control",
                 inVolumeEnabled ? "Enabling" : "Disabling");
    ...
    {
        DebugMsg("WC_Device::SetEnabledControls: %s the mute control",
                 inMuteEnabled ? "Enabling" : "Disabling");
...
void WC_Device::SetSampleRate(Float64 inSampleRate, bool force)
{
    // We try to support any sample rate a real output device might.
    ThrowIf(inSampleRate < 1.0,
            CAException(kAudioDeviceUnsupportedFormatError),
            "WC_Device::SetSampleRate: unsupported sample rate");
    ...
    {
        DebugMsg("WC_Device::SetSampleRate: Changing the sample rate from %f to %f",
    ...
    {
        DebugMsg("WC_Device::SetSampleRate: The sample rate is already set to %f", inSampleRate);
    }
}

bool    WC_Device::IsStreamID(AudioObjectID inObjectID) const noexcept
{
...
void	WC_Device::AddClient(const AudioServerPlugInClientInfo* inClientInfo)
{
    DebugMsg("WC_Device::AddClient: Adding client %u (%s)",
    ...
    // since that's the mutex ApplyClientEQ's caller already holds -- see WC_Device.h.
    {
        CAMutex::Locker theIOLocker(mIOMutex);
...
void	WC_Device::RemoveClient(const AudioServerPlugInClientInfo* inClientInfo)
{
    DebugMsg("WC_Device::RemoveClient: Removing client %u (%s)",
             inClientInfo->mClientID,
             CFStringGetCStringPtr(inClientInfo->mBundleID, kCFStringEncodingUTF8));
...
void	WC_Device::PerformConfigChange(UInt64 inChangeAction, void* inChangeInfo)
{
	#pragma unused(inChangeInfo)
    DebugMsg("WC_Device::PerformConfigChange: inChangeAction = %llu", inChangeAction);

    // Apply a change requested with WC_PlugIn::Host_RequestDeviceConfigurationChange. See
...
void	WC_Device::AbortConfigChange(UInt64 inChangeAction, void* inChangeInfo)
{
