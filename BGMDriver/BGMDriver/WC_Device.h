...
// SuperClass Includes
#include "WC_AbstractDevice.h"

// Local Includes
#include "BGM_Types.h"
#include "WC_Clients.h"
#include "WC_TaskQueue.h"
#include "WC_AudibleState.h"
#include "WC_Stream.h"
#include "WC_VolumeControl.h"
#include "WC_MuteControl.h"
#include "WC_Biquad.h"
...
class WC_Device
:
	public WC_AbstractDevice
{

#pragma mark Construction/Destruction
    
public:
    static WC_Device&			GetInstance();
    static WC_Device&			GetUISoundsInstance();
    
private:
    static void					StaticInitializer();

protected:
                                WC_Device(AudioObjectID inObjectID,
...
private:
    ...
    // live WC_AppEQ processor for that client in mClientEQProcessors. Not const, since it
    // mutates that processor's filter state and possibly its cached gains (only when they've
    // actually changed -- see WC_AppEQ::SetAllBandGainsDB).
    //
...
public:
	/*!
	 ...
	 See WC_Device::PerformConfigChange and RequestDeviceConfigurationChange in AudioServerPlugIn.h.
	 */
...
private:
     ...
     */
	const WC_Object&		    GetOwnedObjectByID(AudioObjectID inObjectID) const;
	WC_Object&                 GetOwnedObjectByID(AudioObjectID inObjectID);
    ...
    /*!
     ...
     for the device. See WC_Device::RequestEnabledControls, WC_Device::PerformConfigChange and
...
public:
    ...
    /*! Cancel a change requested with WC_PlugIn::Host_RequestDeviceConfigurationChange. */
	void						AbortConfigChange(UInt64 inChangeAction, void* __nullable inChangeInfo);

private:
    static pthread_once_t		sStaticInitializer;
    static WC_Device* __nonnull    sInstance;
    static WC_Device* __nonnull    sUISoundsInstance;
    ...
    // Live per-app EQ filter state, keyed by client ID. Deliberately NOT inside WC_Client (see
    ...
    std::map<UInt32, WC_AppEQ> mClientEQProcessors;
    ...
    // WC_Device.cpp), which silently made AppEQ undiscoverable via property introspection. Both
    ...
    // driver targets; WC_Device's constructor confirms this at runtime rather than assuming it.
    ...
    WC_Stream                  mInputStream;
    WC_Stream                  mOutputStream;

    WC_AudibleState            mAudibleState;
    ...
    WC_VolumeControl			mVolumeControl;
	WC_MuteControl				mMuteControl;
