...
//        been changed at all so far, except to replace "SA_Object" with "WC_Object".
...
//
//	It is important that a thread retain and hold a reference while it is using an WC_Object and 
//	that the reference be released promptly when the thread is finished using the object. By
//	assuming this, a WC_Object can minimize the amount of locking it needs to do. In particular,
...
public:
						WC_Object(AudioObjectID inObjectID, AudioClassID inClassID, AudioClassID inBaseClassID, AudioObjectID inOwnerObjectID);
...
private:
						WC_Object(const WC_Object&);
	WC_Object&			operator=(const WC_Object&);
