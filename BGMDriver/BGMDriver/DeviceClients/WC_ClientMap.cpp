...
void    WC_ClientMap::AddClient(WC_Client inClient)
{
    ...
    {
        DebugMsg("WC_ClientMap::AddClient: Found previous volume %f and pan %d for client %u",
...
void    WC_ClientMap::AddClientToShadowMaps(WC_Client inClient)
{
    ThrowIf(mClientMapShadow.count(inClient.mClientID) != 0,
            WC_InvalidClientException(),
            "WC_ClientMap::AddClientToShadowMaps: Tried to add client whose client ID was already in use");
    ...
    WC_Client& clientInMap = mClientMapShadow.at(inClient.mClientID);
...
WC_Client    WC_ClientMap::RemoveClient(UInt32 inClientID)
{
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
    ...
    ThrowIf(theClientItr == mClientMapShadow.end(),
            WC_InvalidClientException(),
            "WC_ClientMap::RemoveClient: Could not find client to be removed");
    
    WC_Client theClient = theClientItr->second;
...
bool    WC_ClientMap::GetClientRT(UInt32 inClientID, WC_Client* outClient) const
{
    CAMutex::Locker theMapsLocker(mMapsMutex);
    return GetClient(mClientMap, inClientID, outClient);
}

bool    WC_ClientMap::GetClientNonRT(UInt32 inClientID, WC_Client* outClient) const
{
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
...
//static
bool    WC_ClientMap::GetClient(const std::map<UInt32, WC_Client>& inClientMap, UInt32 inClientID, WC_Client* outClient)
{
...
std::vector<WC_Client> WC_ClientMap::GetClientsByPID(pid_t inPID) const
{
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
    
    std::vector<WC_Client> theClients;
...
void    WC_ClientMap::UpdateMusicPlayerFlags(pid_t inMusicPlayerPID)
{
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
    
    auto theIsMusicPlayerTest = [&] (WC_Client theClient) {
...
void    WC_ClientMap::UpdateMusicPlayerFlags(CACFString inMusicPlayerBundleID)
{
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
...
void    WC_ClientMap::UpdateMusicPlayerFlagsInShadowMaps(std::function<bool(WC_Client)> inIsMusicPlayerTest)
{
    for(auto& theItr : mClientMapShadow)
    {
        WC_Client& theClient = theItr.second;
...
CACFArray   WC_ClientMap::CopyClientRelativeVolumesAsAppVolumes(CAVolumeCurve inVolumeCurve) const
{
...
void    WC_ClientMap::CopyClientIntoAppVolumesArray(WC_Client inClient, CAVolumeCurve inVolumeCurve, CACFArray& ioAppVolumes) const
{
...
CACFArray   WC_ClientMap::CopyClientEQGainsAsAppEQ() const
{
    // Since this is a read-only, non-real-time operation, we can read from the shadow maps to avoid
    // locking the main maps.
    CAMutex::Locker theShadowMapsLocker(mShadowMapsMutex);
...
void    WC_ClientMap::CopyClientIntoAppEQArray(WC_Client inClient, CACFArray& ioAppEQ) const
{
...
template <typename T>
std::vector<WC_Client*> * _Nullable GetClientsFromMap(std::map<T, std::vector<WC_Client*>> & map, T key) {
...
std::vector<WC_Client*> * _Nullable WC_ClientMap::GetClients(pid_t inAppPid) {
    return GetClientsFromMap(mClientMapByPIDShadow, inAppPid);
}

std::vector<WC_Client*> * _Nullable WC_ClientMap::GetClients(CACFString inAppBundleID) {
...
void ShowSetRelativeVolumeMessage(pid_t inAppPID, WC_Client* theClient);
void ShowSetRelativeVolumeMessage(CACFString inAppBundleID, WC_Client* theClient);

void ShowSetRelativeVolumeMessage(pid_t inAppPID, WC_Client* theClient) {
    (void)inAppPID;
    (void)theClient;
    DebugMsg("WC_ClientMap::ShowSetRelativeVolumeMessage: Set volume %f for client %u by pid (%d)",
...
void ShowSetRelativeVolumeMessage(CACFString inAppBundleID, WC_Client* theClient) {
    (void)inAppBundleID;
    (void)theClient;
    DebugMsg("WC_ClientMap::ShowSetRelativeVolumeMessage: Set volume %f for client %u by bundle ID (%s)",
             theClient->mRelativeVolume,
             theClient->mClientID,
...
//bool WC_ClientMap::SetClientsRelativeVolume(pid_t inAppPID, Float32 inRelativeVolume) {
//    return SetClientsRelativeVolumeT<pid_t>(inAppPID, inRelativeVolume);
//}

//bool WC_ClientMap::SetClientsRelativeVolume(CACFString inAppBundleID, Float32 inRelativeVolume) {
//    return SetClientsRelativeVolumeT<CACFString>(inAppBundleID, inRelativeVolume)
//}

//template <typename T>
//bool WC_ClientMap::SetClientsRelativeVolume(T searchKey, Float32 inRelativeVolume)

bool WC_ClientMap::SetClientsRelativeVolume(pid_t searchKey, Float32 inRelativeVolume)
{
    ...
    auto theSetVolumesInShadowMapsFunc = [&] {
        ...
        {
            for(WC_Client* theClient : *theClients)
            {
...
bool WC_ClientMap::SetClientsRelativeVolume(CACFString searchKey, Float32 inRelativeVolume)
{
    bool didChangeVolume = false;
...
bool WC_ClientMap::SetClientsPanPosition(pid_t searchKey, SInt32 inPanPosition)
{
...
bool WC_ClientMap::SetClientsPanPosition(CACFString searchKey, SInt32 inPanPosition)
{
    bool didChangePanPosition = false;
...
bool WC_ClientMap::SetClientsEQBandGains(pid_t searchKey,
...
bool WC_ClientMap::SetClientsEQBandGains(CACFString searchKey,
                                           const std::array<Float32, WC_AppEQ::kNumBands>& inGainsDB)
{
    bool didChangeGains = false;
...
void    WC_ClientMap::SwapInShadowMaps()
{
    mTaskQueue->QueueSync_SwapClientShadowMaps(this);
}

void    WC_ClientMap::SwapInShadowMapsRT()
{
#if DEBUG
    // This method should only be called by the realtime worker thread in WC_TaskQueue. The only safe way to call it is on a realtime
    ...
    mTaskQueue->AssertCurrentThreadIsRTWorkerThread("WC_ClientMap::SwapInShadowMapsRT");
