...
public:
                                        WC_Clients(AudioObjectID inOwnerDeviceID, WC_TaskQueue* inTaskQueue);
                                        ~WC_Clients() = default;
    // Disallow copying. (It could make sense to implement these in future, but we don't need them currently.)
                                        WC_Clients(const WC_Clients&) = delete;
                                        WC_Clients& operator=(const WC_Clients&) = delete;
    ...
    // live WC_AppEQ processor -- WC_Clients/WC_ClientMap/WC_Client never own the actual
