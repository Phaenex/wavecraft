...
WC_Clients::WC_Clients(AudioObjectID inOwnerDeviceID, WC_TaskQueue* inTaskQueue)
:
...
void    WC_Clients::AddClient(WC_Client inClient)
{
    ...
    {
        DebugMsg("WC_Clients::AddClient: Adding music player client. mClientID = %u", inClient.mClientID);
    }
...
void    WC_Clients::RemoveClient(const UInt32 inClientID)
{
    CAMutex::Locker theLocker(mMutex);
    
    WC_Client theRemovedClient = mClientMap.RemoveClient(inClientID);
...
//       manages it (the functions below, WC_Device::mInactivityTimer, etc.) if we split BGMDevice into a separate
...
bool    WC_Clients::ClientsOtherThanBGMAppRunningIO() const noexcept
{
...
void    WC_Clients::RecordNonBGMAppIO(const UInt32 inClientID) noexcept
{
...
void    WC_Clients::SendIORunningPropertyNotification() noexcept
{
    ...
    {
        DebugMsg("WC_Clients::SendIORunningPropertyNotification: isRunningSomewhereOtherThanBGMApp changed to %s",
                 isRunning ? "true" : "false");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theAddr = kBGMRunningSomewhereOtherThanBGMAppAddress;
            WC_PlugIn::Host_PropertiesChanged(mOwnerDeviceID, 1, &theAddr);
        });
    }
}

bool    WC_Clients::StartIO(UInt32 inClientID)
{
    CAMutex::Locker theLocker(mMutex);

    ThrowIf(mStartCount == UINT64_MAX,
            CAException(kAudioHardwareIllegalOperationError),
            "WC_Clients::StartIO: mStartCount overflow");
    ...
    if(isFirstClient)
    {
        ...
        BGMAssert(mInactivityTimer == nullptr, "WC_Clients::StartIO: Inactivity timer already running");
        ...
        dispatch_source_set_event_handler(theTimer, ^{
            BGMLogAndSwallowExceptions("WC_Clients::StartIO inactivityTimer", ([&] {
...
bool    WC_Clients::StopIO()
{
    CAMutex::Locker theLocker(mMutex);

    ThrowIf(mStartCount == 0,
            CAException(kAudioHardwareIllegalOperationError),
            "WC_Clients::StopIO: mStartCount underflow");
    ...
    if(isLastClient)
    {
        ...
        if(wasRunning)
        {
            DebugMsg("WC_Clients::StopIO: isRunningSomewhereOtherThanBGMApp changed to false");
            CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
                AudioObjectPropertyAddress theAddr = kBGMRunningSomewhereOtherThanBGMAppAddress;
                WC_PlugIn::Host_PropertiesChanged(mOwnerDeviceID, 1, &theAddr);
            });
...
bool    WC_Clients::IsIORunning() const
{
    CAMutex::Locker theLocker(mMutex);
...
bool    WC_Clients::SetMusicPlayer(const pid_t inPID)
{
    ThrowIf(inPID < 0, WC_InvalidClientPIDException(), "WC_Clients::SetMusicPlayer: Invalid music player PID");
    ...
    DebugMsg("WC_Clients::SetMusicPlayer: Setting music player by PID. inPID=%d", inPID);
...
bool    WC_Clients::SetMusicPlayer(const CACFString inBundleID)
{
    Assert(inBundleID.IsValid(), "WC_Clients::SetMusicPlayer: Invalid CACFString given as bundle ID");
    ...
    DebugMsg("WC_Clients::SetMusicPlayer: Setting music player by bundle ID. inBundleID=%s",
...
bool    WC_Clients::IsMusicPlayerRT(const UInt32 inClientID) const
{
    WC_Client theClient;
...
Float32 WC_Clients::GetClientRelativeVolumeRT(UInt32 inClientID) const
{
    WC_Client theClient;
    bool didGetClient = mClientMap.GetClientRT(inClientID, &theClient);
...
SInt32 WC_Clients::GetClientPanPositionRT(UInt32 inClientID) const
{
    WC_Client theClient;
    bool didGetClient = mClientMap.GetClientRT(inClientID, &theClient);
...
std::array<Float32, WC_AppEQ::kNumBands> WC_Clients::GetClientEQBandGainsRT(UInt32 inClientID) const
{
    WC_Client theClient;
    bool didGetClient = mClientMap.GetClientRT(inClientID, &theClient);
    return (didGetClient ? theClient.mEQBandGainsDB
                          : std::array<Float32, WC_AppEQ::kNumBands> {});
}

bool    WC_Clients::SetClientsRelativeVolumes(const CACFArray inAppVolumes)
{
    ...
    for(UInt32 i = 0; i < inAppVolumes.GetNumberItems(); i++)
    {
        ...
        BGMAssert(didFindPID || theAppBundleID.IsValid(),
                  "WC_Clients::SetClientsRelativeVolumes: No PID or bundle ID");
        ...
        ThrowIf(!didGetVolume && !didGetPanPosition,
                WC_InvalidClientRelativeVolumeException(),
                "WC_Clients::SetClientsRelativeVolumes: No volume or pan position in request");
    }
...
bool    WC_Clients::SetClientsEQ(const CACFArray inAppEQ)
{
    ...
    for(UInt32 i = 0; i < inAppEQ.GetNumberItems(); i++)
    {
        ...
        BGMAssert(didFindPID || theAppBundleID.IsValid(),
                  "WC_Clients::SetClientsEQ: No PID or bundle ID");
        (void)didFindPID;  // Suppress unused variable warning in release builds.

        // Get the band gains array from the dict. ValidateAppEQProperty (in WC_Device.cpp)
        ...
        std::array<Float32, WC_AppEQ::kNumBands> theGainsDB;
        for(UInt32 band = 0; band < WC_AppEQ::kNumBands; band++)
        {
            ...
            // WC_Biquad::SetParameters clamps again -- cheap, and this is the one place a
