...
WC_Control::WC_Control(AudioObjectID inObjectID,
...
:
    WC_Object(inObjectID, inClassID, inBaseClassID, inOwnerObjectID),
    mScope(inScope),
    mElement(inElement)
{
}

bool    WC_Control::HasProperty(AudioObjectID inObjectID,
                                 pid_t inClientPID,
...
bool    WC_Control::IsPropertySettable(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress) const
{
    CheckObjectID(inObjectID);
...
UInt32  WC_Control::GetPropertyDataSize(AudioObjectID inObjectID,
                                         pid_t inClientPID,
...
void    WC_Control::GetPropertyData(AudioObjectID inObjectID,
                                     pid_t inClientPID,
                                     const AudioObjectPropertyAddress& inAddress,
                                     UInt32 inQualifierDataSize,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioControlPropertyScope:
            ...
            ThrowIf(inDataSize < sizeof(AudioObjectPropertyScope),
                    ...
                    "WC_Control::GetPropertyData: not enough space for the return value of "
...
void    WC_Control::CheckObjectID(AudioObjectID inObjectID) const
{
    ThrowIf(inObjectID == kAudioObjectUnknown || inObjectID != GetObjectID(),
            CAException(kAudioHardwareBadObjectError),
            "WC_Control::CheckObjectID: wrong audio object ID for the control");
}
