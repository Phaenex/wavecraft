...
private:
    ...
    {
    public:
                                        WC_Task(BGM_TaskID inTaskID = kBGMTaskUninitialized, bool inIsSync = false, UInt64 inArg1 = 0, UInt64 inArg2 = 0) : mNext(NULL), mTaskID(inTaskID), mIsSync(inIsSync), mArg1(inArg1), mArg2(inArg2) { };
        ...
        WC_Task* __nullable &          next() { return mNext; }
        WC_Task* __nullable            mNext;
...
public:
                                        ...
                                        ~WC_TaskQueue();
                                        // Disallow copying
                                        WC_TaskQueue(const WC_TaskQueue&) = delete;
                                        WC_TaskQueue& operator=(const WC_TaskQueue&) = delete;
    
private:
    static UInt32                       NanosToAbsoluteTime(UInt32 inNanos);
    
public:
    void                                QueueSync_SwapClientShadowMaps(WC_ClientMap* inClientMap);
...
private:
    ...
    void                                QueueOnNonRealtimeThread(WC_Task inTask);
...
private:
    ...
    void                                WorkerThreadProc(semaphore_t inWorkQueuedSemaphore, semaphore_t inSyncTaskCompletedSemaphore, TAtomicStack<WC_Task>* inTasks, TAtomicStack2<WC_Task>* __nullable inFreeList, std::function<bool(WC_Task*)> inProcessTask);
    
    // These return true when the thread should be stopped
    bool                                ProcessRealTimeThreadTask(WC_Task* inTask);
    bool                                ProcessNonRealTimeThreadTask(WC_Task* inTask);
...
private:
    ...
    TAtomicStack<WC_Task>              mRealTimeThreadTasks;
    TAtomicStack<WC_Task>              mNonRealTimeThreadTasks;
    ...
    TAtomicStack2<WC_Task>             mNonRealTimeThreadTasksFreeList;
