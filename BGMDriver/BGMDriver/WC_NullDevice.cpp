...
pthread_once_t     WC_NullDevice::sStaticInitializer = PTHREAD_ONCE_INIT;
WC_NullDevice*    WC_NullDevice::sInstance = nullptr;

WC_NullDevice&    WC_NullDevice::GetInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sInstance;
}

void    WC_NullDevice::StaticInitializer()
{
    try
    {
        sInstance = new WC_NullDevice;
        // Note that we leave the device inactive initially. BGMApp will activate it when needed.
    }
    catch(...)
    {
        DebugMsg("WC_NullDevice::StaticInitializer: Failed to create the device");
        delete sInstance;
        sInstance = nullptr;
    }
}

WC_NullDevice::WC_NullDevice()
:
    WC_AbstractDevice(kObjectID_Device_Null, kAudioObjectPlugInObject),
...
void    WC_NullDevice::SendDeviceIsAlivePropertyNotifications()
{
    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
        ...
        WC_PlugIn::Host_PropertiesChanged(GetObjectID(), 1, theChangedProperties);
    });
}

#pragma mark Property Operations

bool    WC_NullDevice::HasProperty(AudioObjectID inObjectID,
                                    pid_t inClientPID,
...
bool    WC_NullDevice::IsPropertySettable(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress) const
{
...
UInt32    WC_NullDevice::GetPropertyDataSize(AudioObjectID inObjectID,
                                            pid_t inClientPID,
                                            ...
                                            const void* __nullable inQualifierData) const
{
    ...
    switch(inAddress.mSelector)
    {
        ...
        default:
            theAnswer = WC_AbstractDevice::GetPropertyDataSize(inObjectID,
                                                                inClientPID,
                                                                inAddress,
                                                                inQualifierDataSize,
                                                                inQualifierData);
...
void    WC_NullDevice::GetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        void* outData) const
{
    ...
    // See WC_Device::Device_GetPropertyData for more information about these properties.
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyName:
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    ...
                    "WC_NullDevice::GetPropertyData: not enough space for the return value of "
        ...
        default:
            WC_AbstractDevice::GetPropertyData(inObjectID,
                                                inClientPID,
                                                inAddress,
                                                inQualifierDataSize,
                                                inQualifierData,
...
void    WC_NullDevice::SetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        const void* inData)
{
    ...
    {
        WC_AbstractDevice::SetPropertyData(inObjectID,
                                            inClientPID,
                                            inAddress,
                                            inQualifierDataSize,
                                            inQualifierData,
...
void    WC_NullDevice::StartIO(UInt32 inClientID)
{
    ...
    if(mClientsDoingIO == 0)
    {
        ...
        // Send notifications.
        DebugMsg("WC_NullDevice::StartIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
            WC_PlugIn::Host_PropertiesChanged(kObjectID_Device_Null, 1, theChangedProperty);
        });
    }
...
void    WC_NullDevice::StopIO(UInt32 inClientID)
{
    #pragma unused (inClientID)
    ...
    ThrowIf(mClientsDoingIO == 0,
            ...
            "WC_NullDevice::StopIO: Underflowed mClientsDoingIO");
    ...
    {
        // Send notifications.
        DebugMsg("WC_NullDevice::StopIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
...
void    WC_NullDevice::GetZeroTimeStamp(Float64& outSampleTime,
                                         UInt64& outHostTime,
                                         UInt64& outSeed)
{
    CAMutex::Locker theIOLocker(mIOMutex);
    ...
    // clock similar to the loopback clock in WC_Device.
...
void    WC_NullDevice::WillDoIOOperation(UInt32 inOperationID,
                                          bool& outWillDo,
...
void    WC_NullDevice::DoIOOperation(AudioObjectID inStreamObjectID,
                                      UInt32 inClientID,
