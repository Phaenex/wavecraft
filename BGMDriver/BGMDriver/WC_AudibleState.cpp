...
WC_AudibleState::WC_AudibleState()
:
...
BGMDeviceAudibleState   WC_AudibleState::GetState() const noexcept
{
    CAMemoryBarrier();  // Probably unnecessary.
    return mState;
}

void    WC_AudibleState::Reset() noexcept
{
    mState = kBGMDeviceIsSilent;
...
void    WC_AudibleState::UpdateWithClientIO(bool inClientIsMusicPlayer,
...
bool    WC_AudibleState::UpdateWithMixedIO(UInt32 inIOBufferFrameSize,
                                            Float64 inOutputSampleTime,
                                            const Float32* inBuffer)
{
...
bool    WC_AudibleState::RecalculateState(Float64 inEndFrameSampleTime)
{
    ...
    {
        DebugMsg("WC_AudibleState::RecalculateState: Changing "
...
// static
bool    WC_AudibleState::BufferIsAudible(UInt32 inIOBufferFrameSize, const Float32* inBuffer)
{
