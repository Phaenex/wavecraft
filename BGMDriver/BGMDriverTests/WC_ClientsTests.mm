...
@interface WC_ClientsTests : XCTestCase

@end

@implementation WC_ClientsTests {
    WC_Clients* clients;
}
...
- (void)setUp {
    ...
    clients = new WC_Clients(kAudioObjectUnknown, &taskQueue);
}
...
- (void)testSetMusicPlayerInvalidPID {
    BGMShouldThrow<WC_InvalidClientPIDException>(self, [=](){
...
- (void)testClientEQGainsDefaultToFlat {
    ...
    std::array<Float32, WC_AppEQ::kNumBands> gains = clients->GetClientEQBandGainsRT(client1Info.mClientID);
...
- (void)testSetClientsEQByBundleIDRoundTrips {
    ...
    for (int i = 0; i < WC_AppEQ::kNumBands; i++) {
...
- (void)testSetClientsEQByPIDRoundTrips {
    ...
    std::array<Float32, WC_AppEQ::kNumBands> gains = clients->GetClientEQBandGainsRT(client2Info.mClientID);
    for (Float32 gain : gains) {
...
- (void)testSetClientsEQClampsOutOfRangeGains {
    ...
    for (int i = 2; i < WC_AppEQ::kNumBands; i++) {
